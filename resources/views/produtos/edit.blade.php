...
<section class="content-header Produtos_edit">
    <h1>Produtos </h1>
    <!--@if(!$isPublic)
    <ol class="breadcrumb">
        <li><a href="{{ URL('/') }}">Home</a></li>
        <li><a href="{{ URL('/') }}/produtos">Produtos</a></li>
        <li class="active">#{{$produtos->id}}</li>
    </ol>
    @endif-->
</section>

<section class="content Produtos_edit">
...
<div class="box">
    ...
    @if(\App\Models\Permissions::permissaoUsuario(\Auth::user(), "$controller@destroy"))
        <div class="row" style="position: absolute; right: 0; padding: 5px;">
            <div class="col-md-12">
                <form id="form-destroy" method="POST" action="{{ route('produtos.destroy', $produtos->id) }}" accept-charset="UTF-8">
                    {!! csrf_field() !!}
                    {!! method_field('DELETE') !!}
    ...
    {!! Form::open(['url' => "produtos/$produtos->id", 'method' => 'put', 'novalidate'=> true, 'enctype' => 'multipart/form-data', 'accept-charset' => 'utf-8', 'id' => 'form_edit_produtos']) !!}

        @if(\Request::get('modal'))
            {!! Form::hidden('modal-close', 1) !!}
        @endif
        {!! Form::hidden('id', $produtos->id) !!}
        ...
        <div class="box-body" id="div_produtos">
            ...
            <div size="12" class="inputbox col-md-12">
                <div class="form-group">
                    {!! Form::label('','Produto') !!}
                    {!! Form::text('produto', $produtos->produto, ['class' => 'form-control' , "id" => "input_produto", "style" => "text-transform: uppercase;", "oninput" => "this.value = this.value.toUpperCase();"]) !!}
                </div>
            </div>
                ...
                @if(\App\Models\Permissions::permissaoModerador(\Auth::user()))
                    <div class="col-md-12">
                        <div class="form-group">
                            ...
                            {!! Form::select('r_auth', $parserList, $produtos->r_auth, ['class' => 'form-control']) !!}
                        </div>
                    </div>
                @endif
