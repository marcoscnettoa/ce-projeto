...
@section('content')
    ...
    <section class="content">
        ...
        <div class="box-header" style="background-color: #fff; padding-top: 15px">
            <div class="row">
                <div class="col-md-12">
                    <form action="{{ URL('/') }}/mra_g_iugu/mra_g_iugu_clientes/iugu_load" method="POST">
                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
                        <input type="hidden" name="iugu_load" value="1">
                        <button type="submit" class="btn btn-info submitbtn" style="float: right;">
                            <span class="glyphicon glyphicon glyphicon-floppy-save"></span>&nbsp; Carregar Clientes Iugu
                        </button>
                    </form>
                </div>
            </div>
            <form action="{{ URL('/') }}/mra_g_iugu/mra_g_iugu_clientes/filter" method="POST" style="display:none;">
                <input type="hidden" name="_token" value="{{ csrf_token() }}">
                <div class="row">
                ...
                <div class="row">
                    <div class="col-md-3 btnsFiltro">
                        <a href="{{ URL('/mra_g_iugu/mra_g_iugu_clientes') }}" class="btn btn-default" style="float: left; margin-right: 5px;"><i class="glyphicon glyphicon-trash"></i> Limpar</a>
        ...
        <div class="box">
            ...
            <div class="box-body table-responsive">
                ...
                <table id="<?php echo (!$export_enable_btns ? 'datatable-no-buttons' : 'datatable'); ?>" class="display table-striped table-bordered stripe" cellspacing="0" width="100%">
                    <thead>
                        <tr>
                            <th style="white-space: nowrap;" class="text-warning">ID Iugu Cliente</th>
                    ...
                    <tbody>
                        @foreach($MRAGIuguClientes as $value)
                            <tr>
                                <td>{{$value->iugu_customer_id}}</td>
                                ...
                                @if($auth_user__actions_enable_btns)
                                    <td>
                                        @if($permissaoUsuario_auth_user__controller_update)
                                            <a style="float: left; margin-right: 5px;" href="{{ URL('/') }}/mra_g_iugu/mra_g_iugu_clientes/{{$value->id}}/edit" alt="Editar" title="Editar" class="btn btn-default">
                                                <span class="glyphicon glyphicon-edit"></span>
                                            </a>
                                        @endif
                                        {{--@if(0)
                                            @if($permissaoUsuario_auth_user__controller_copy)
                                                <a style="float: left; margin-right: 5px;" href="{{ URL('/') }}/mra_g_iugu/mra_g_iugu_clientes/{{$value->id}}/copy" alt="Duplicar linha" title="Duplicar linha" class="btn btn-default" onclick="return confirm('Tem certeza que quer duplicar a linha?')" >
                                                    <span class="glyphicon glyphicon-copy"></span>
                                                </a>
                                            @endif
                                        @endif--}}
                                        {{--@if($permissaoUsuario_auth_user__controller_show)
                                            <a style="float: left; margin-right: 5px;" href="{{ URL('/') }}/mra_g_iugu/mra_g_iugu_clientes/{{$value->id}}" alt="Visualizar" title="Visualizar" class="btn btn-default">
                                                <span class="glyphicon glyphicon-eye-open"></span>
                                            </a>
                                        @endif--}}
                                        @if($permissaoUsuario_auth_user__controller_destroy)
                                            <form style="float: left; margin-right: 5px;" method="POST" action="{{ route('mra_g_iugu_clientes.destroy', $value->id) }}" accept-charset="UTF-8">
                                                {!! csrf_field() !!}
                                                {!! method_field('DELETE') !!}
                ...
                <div class="form-group form-group-btn-index">
                    ...
                    @if(\App\Models\Permissions::permissaoUsuario(\Auth::user(), "$controller@store"))
                        <a href="{{ URL('/') }}/mra_g_iugu/mra_g_iugu_clientes/create" class="btn btn-default right form-group-btn-index-cadastrar"><i class="glyphicon glyphicon-plus"></i> Cadastrar</a>
                    @endif
                </div>
            </div>
        </div>
