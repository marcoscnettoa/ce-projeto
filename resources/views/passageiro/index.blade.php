...
<section class="content-header Passageiro_index">
    <h1>Passageiro</h1>
</section>
...
<section class="content">
    ...
    @if($exibe_filtros)
        <div class="box-header" style="background-color: #fff; padding-top: 15px">
            <form action="{{ URL('/') }}/passageiro/filter" class="form_filter form_filter_passageiro" method="POST">
...
<div class="form-group">
                    {!! Form::label('','Nome') !!}
                    {!! Form::text('nome', (!empty(\Request::post('nome'))?\Request::post('nome'):null), ['class' => 'form-control' ]) !!}
</div>
</div>
<div size="3" class="inputbox col-md-3">
<div class="form-group">
                    {!! Form::label('','CPF') !!}
                    {!! Form::text('cpf', (!empty(\Request::post('cpf'))?\Request::post('cpf'):null), ['class' => 'form-control cpf' ]) !!}
</div>
</div>
<div size="3" class="inputbox col-md-3">
<div class="form-group">
                    ...
                    {!! Form::email('e_mail', (!empty(\Request::post('e_mail'))?\Request::post('e_mail'):null), ['class' => 'form-control' ]) !!}
                    </div>
</div>
</div>
<div size="3" class="inputbox col-md-3">
<div class="form-group">
                    ...
                    {!! Form::text('telefone', (!empty(\Request::post('telefone'))?\Request::post('telefone'):null), ['class' => 'form-control telefone' ]) !!}
                    </div>
</div>
</div>
<div size="3" class="inputbox col-md-3">
<div class="form-group">
                    ...
                    {!! Form::text('telefone_2', (!empty(\Request::post('telefone_2'))?\Request::post('telefone_2'):null), ['class' => 'form-control telefone' ]) !!}
                    </div>
</div>
</div>
<div size="3" class="inputbox col-md-3">
<div class="form-group">
                    ...
                    {!! Form::text('passaporte', (!empty(\Request::post('passaporte'))?\Request::post('passaporte'):null), ['class' => 'form-control' ]) !!}
</div>
</div>

                    <div class="col-md-12 btnsFiltro">
                        <a href="{{ URL('/passageiro') }}" class="btn btn-xs btn-default" style="float: left; margin-right: 5px;"><i class="glyphicon glyphicon-trash"></i> Limpar</a>
            ...
            <div class="box-body">
                ...
                @endphp
                             <td style="width:100px; white-space: nowrap;">
                                ...
                                @if($permissaoUsuario_auth_user__controller_update)
                                    <a style="float:none;" href="{{ URL('/') }}/passageiro/{{$value->id}}/edit" alt="Editar" title="Editar" class="btn btn-xs btn-default">
                                ...
                                @if(1)
                                    @if($permissaoUsuario_auth_user__controller_copy)
                                        <a style="float:none;" href="{{ URL('/') }}/passageiro/{{$value->id}}/copy" alt="Duplicar linha" title="Duplicar linha" class="btn btn-xs btn-default" onclick="return confirm('Tem certeza que quer duplicar a linha?')" >
                                ...
                                @if($permissaoUsuario_auth_user__controller_show)
                                    <a style="float:none;" href="{{ URL('/') }}/passageiro/{{$value->id}}" alt="Visualizar" title="Visualizar" class="btn btn-xs btn-default">
                                        <span class="glyphicon glyphicon-eye-open"></span>
                                    </a>
                                @endif

                                @if($permissaoUsuario_auth_user__controller_destroy)
                                    <form style="display:inline-block;" method="POST" action="{{ route('passageiro.destroy', $value->id) }}" accept-charset="UTF-8">
                                        {!! csrf_field() !!}
                                        {!! method_field('DELETE') !!}
                ...
                <table id="datatable-no-buttons" class="display table-striped table-bordered stripe" cellspacing="0" width="100%">
                    <thead>
                        <tr>
                            ...
                            <th class="passageiro_td_nome" style="white-space: nowrap;">Nome</th>
                            <th class="passageiro_td_cpf" style="white-space: nowrap;">CPF</th>
                            <th class="passageiro_td_e_mail" style="white-space: nowrap;">E-mail</th>
                            <th class="passageiro_td_data_de_nascimento" style="white-space: nowrap;">Data de Nascimento</th>
                            <th class="passageiro_td_telefone" style="white-space: nowrap;">Telefone</th>
                            <th class="passageiro_td_telefone_2" style="white-space: nowrap;">Telefone 2</th>
                            <th class="passageiro_td_passaporte" style="white-space: nowrap;">Passaporte</th>
                    ...
                    <tbody>
                        @foreach($passageiro as $value)
                            ...
                            <tr>
                                ...
                                @endif
                                <td style="text-transform: uppercase;"  class="passageiro_td_nome">{{$value->nome}}</td>
                                <td style=' white-space: nowrap; '  class='passageiro_td_cpf'>{{$value->cpf}}</td>
                                <td style=' white-space: nowrap; '  class='passageiro_td_e_mail'>{{$value->e_mail}}</td>
                                <td style=' white-space: nowrap; '  class='passageiro_td_data_nascimento'>{{(!empty($value->data_nascimento)?\App\Helper\Helper::H_Data_DB_ptBR($value->data_nascimento):'---')}}</td>
                                <td style=' white-space: nowrap; '  class='passageiro_td_telefone'>{{$value->telefone}}</td>
                                <td style=' white-space: nowrap; '  class='passageiro_td_telefone_2'>{{$value->telefone_2}}</td>
                                <td style=' white-space: nowrap; '  class='passageiro_td_passaporte'>{{$value->passaporte}}</td>
                                @if($auth_user__actions_enable_btns && $defaultPositionActions == 'right')
                                {{
                                    LOAD_ACTIONS_TD(
                                        $auth_user__actions_enable_btns,
                ...
                <div class="form-group form-group-btn-index">
                    ...
                    @if(\App\Models\Permissions::permissaoUsuario(\Auth::user(), "$controller@store") OR $isPublic)
                        <a href="{{ URL('/') }}/passageiro/create" class="btn btn-xs btn-default right form-group-btn-index-cadastrar"><i class="glyphicon glyphicon-plus"></i> Cadastrar</a>
                    @endif
                </div>
            </div>
        </div>
    @else
        @include('passageiro.kanban', [
