@php
    ...
    $td_colunas_fields      = json_decode('[{"name":"Nome do Template","type":"text","className":"NomeDoTemplate","cleanName":"nome_do_template"},{"name":"Template","type":"html","disabled":false,"note":null,"value":null,"class":"html","actions_advanced":true,"hidden_view":true,"className":"Template","cleanName":"template"}]',true);
    ...
    foreach($templates as $key => $list){
        ...
        $quadros[$list->$kanban_field]['item'][$key]['title']     .= '
            <div class="kanban-buttons text-right">
                <button modulo="templates" modulo_id="'.$list->id.'" modulo_method="GET" modulo_url="templates/'.$list->id.'/edit" modulo_redirect="true" class="btn btn-xs btn-default kanban-btn-edit"><i class="glyphicon glyphicon-edit" title="Editar"></i></button>
                <button modulo="templates" modulo_id="'.$list->id.'" modulo_method="GET" modulo_url="templates/'.$list->id.'/copy" modulo_redirect="true" class="btn btn-xs btn-default kanban-btn-copy"><i class="glyphicon glyphicon-copy" title="Duplicar linha"></i></button>
                <button modulo="templates" modulo_id="'.$list->id.'" modulo_method="GET" modulo_url="templates/'.$list->id.'" modulo_redirect="true" class="btn btn-xs btn-default kanban-btn-view"><i class="glyphicon glyphicon-eye-open" title="Visualizar"></i></button>
                <button modulo="templates" modulo_id="'.$list->id.'" class="btn btn-xs btn-danger excluir-auto-confirma" style="float:none;margin: 0px;" title="Excluir"><i class="glyphicon glyphicon-trash"></i></button>
            </div>
        ';
    }
...
<div class="form-group form-group-btn-index text-right">
    @if(\App\Models\Permissions::permissaoUsuario(\Auth::user(), "$controller@store") OR $isPublic)
        <a href="{{ URL('/') }}/templates/create" class="btn btn-default form-group-btn-index-cadastrar"><i class="glyphicon glyphicon-plus"></i> Cadastrar</a>
    @endif
</div>
