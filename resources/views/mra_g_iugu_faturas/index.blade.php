...
@section('content')
    ...
    <section class="content">
        ...
        <div class="box-header" style="background-color: #fff; padding-top: 15px">
            {{--<div class="row">
                <div class="col-md-12">
                    <form action="{{ URL('/') }}/mra_g_iugu/mra_g_iugu_faturas/iugu_load" method="POST">
                        <input type="hidden" name="_token" value="{{ csrf_token() }}">
                        <input type="hidden" name="iugu_load" value="1">
                        <button type="submit" class="btn btn-info submitbtn" style="float: right;">
                            <span class="glyphicon glyphicon glyphicon-floppy-save"></span>&nbsp; Carregar Faturas Iugu
                        </button>
                    </form>
                </div>
            </div>--}}
            <form action="{{ URL('/') }}/mra_g_iugu/mra_g_iugu_faturas/filter" method="POST" style="display:none;">
                <input type="hidden" name="_token" value="{{ csrf_token() }}">
                <div class="row">
                ...
                <div class="row">
                    <div class="col-md-3 btnsFiltro">
                        <a href="{{ URL('/mra_g_iugu/mra_g_iugu_faturas') }}" class="btn btn-default" style="float: left; margin-right: 5px;"><i class="glyphicon glyphicon-trash"></i> Limpar</a>
        ...
        <div class="box">
            ...
            <div class="box-body table-responsive">
                ...
                <table id="<?php echo (!$export_enable_btns ? 'datatable-no-buttons' : 'datatable'); ?>" class="display table-striped table-bordered stripe" cellspacing="0" width="100%">
                    <thead>
                    <tr>
                        <th style="white-space: nowrap;" class="text-warning">ID Iugu Fatura</th>
                        <th style="white-space: nowrap;">N&deg; Fatura</th>
                        <th style="white-space: nowrap;">Data de Vencimento</th>
                        ...
                        @if($auth_user__actions_enable_btns)
                            <th style="border: none; min-width: 160px;"></th>
                        @endif
                    </tr>
                    </thead>

                    <tbody>
                    @foreach($MRAGIuguFaturas as $value)
                        <tr>
                            <td>{{$value->iugu_invoices_id}}</td>
                            <td style="white-space: nowrap;">{{(!empty($value->iugu_order_id)?$value->iugu_order_id:'---')}}</td>
                            <td style="white-space: nowrap;">{{!empty($value->iugu_due_date)?date('d/m/Y',strtotime($value->iugu_due_date)):'---'}}</td>
                            <td style="white-space: nowrap;">{{(!empty($value->nome)?$value->nome:'---')}}</td>
                            <td style="white-space: nowrap;">{{(!empty($value->tipo)?\App\Http\Controllers\MRA\MRAListas::Get_tipo_pessoa($value->tipo):'---')}}</td>
                            @php
                                $cnpj_cpf = '---';
                                if($value->tipo=='F'){
                                    $cnpj_cpf = $value->cpf;
                            ...
                            @endphp
                            <td style="white-space: nowrap;">{{ $cnpj_cpf }}</td>
                            <td style="white-space: nowrap;">{{(!empty($value->iugu_total_cents)?\App\Helper\Helper::H_Decimal_DB_ptBR($value->iugu_total_cents):'0,00')}}</td>
                            @php
                                $badge_iugu_status = 'badge-default';
                                switch($value->iugu_status){
                                    case 'expired':
                                    case 'authorized':
                                    case 'pending':             $badge_iugu_status = 'badge-warning'; break;
                                    case 'partially_paid':
                                    case 'externally_paid':
                                    case 'paid':                $badge_iugu_status = 'badge-success'; break;
                                    case 'refunded':            $badge_iugu_status = 'badge-info'; break;
                                    case 'canceled':            $badge_iugu_status = 'badge-danger'; break;
                                }

                                $iugu_status  = (!empty($value->iugu_status)?\App\Http\Controllers\MRA\MRAGIugu::Get_invoice_iugu_status($value->iugu_status):'---');
                            @endphp
                            <td style="white-space: nowrap;"><span class="badge {{$badge_iugu_status}} fw-600">{{ $iugu_status }}</span></td>
                            ...
                            @if($auth_user__actions_enable_btns)
                                <td style="white-space: nowrap; vertical-align: top;">
                                    @if($permissaoUsuario_auth_user__controller_show and !empty($value->iugu_secure_url))
                                        <a style="float: left; margin: 2px; margin-right: 5px;" href="{{$value->iugu_secure_url}}" target="_blank" alt="Visualizar Fatura" title="Visualizar Fatura" class="btn btn-info">
                                            <span class="glyphicon glyphicon-list-alt"></span>
                                        </a>
                                    @endif
                                    @if($permissaoUsuario_auth_user__controller_update)
                                        <a style="float: left; margin-right: 5px;" href="{{ URL('/') }}/mra_g_iugu/mra_g_iugu_faturas/{{$value->id}}/edit" alt="Editar" title="Editar" class="btn btn-default">
                                            <span class="glyphicon glyphicon-edit"></span>
                                        </a>
                                    @endif
                                    {{--@if(0)
                                        @if($permissaoUsuario_auth_user__controller_copy)
                                            <a style="float: left; margin-right: 5px;" href="{{ URL('/') }}/mra_g_iugu/mra_g_iugu_faturas/{{$value->id}}/copy" alt="Duplicar linha" title="Duplicar linha" class="btn btn-default" onclick="return confirm('Tem certeza que quer duplicar a linha?')" >
                                                <span class="glyphicon glyphicon-copy"></span>
                                            </a>
                                        @endif
                                    @endif--}}
                                    {{--@if($permissaoUsuario_auth_user__controller_show)
                                        <a style="float: left; margin-right: 5px;" href="{{ URL('/') }}/mra_g_iugu/mra_g_iugu_faturas/{{$value->id}}" alt="Visualizar" title="Visualizar" class="btn btn-default">
                                            <span class="glyphicon glyphicon-eye-open"></span>
                                        </a>
                                    @endif--}}
                                    @if($permissaoUsuario_auth_user__controller_destroy)
                                        <form style="float: left; margin-right: 5px;" method="POST" action="{{ route('mra_g_iugu_faturas.destroy', $value->id) }}" accept-charset="UTF-8">
                                            {!! csrf_field() !!}
                                            {!! method_field('DELETE') !!}
                                            <button type="submit" style="margin:1px;" onclick="return confirm('Tem certeza que quer deletar?')" class="btn btn-danger glyphicon glyphicon-trash"></button>
                                        </form>
                                    @endif
                                </td>
                            @endif
                ...
                <div class="form-group form-group-btn-index">
                    ...
                    @if(\App\Models\Permissions::permissaoUsuario(\Auth::user(), "$controller@store"))
                        <a href="{{ URL('/') }}/mra_g_iugu/mra_g_iugu_faturas/create" class="btn btn-default right form-group-btn-index-cadastrar"><i class="glyphicon glyphicon-plus"></i> Cadastrar</a>
                    @endif
                </div>
            </div>
        </div>
