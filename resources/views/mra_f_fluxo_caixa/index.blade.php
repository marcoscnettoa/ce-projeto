...
@section('content')
    ...
    <section class="content">
            ...
            <div class="row">
                <div class="col-md-4">
                    <div class="small-box v2 bg-success">
                        <div class="inner" style="color: #000;">
                            @php
                                $valor_entradas_get = \App\Models\MRAFFluxoCaixa::Get_ValorTotal_Entradas();
                ...
                <div class="col-md-4">
                    <div class="small-box v2 bg-danger">
                        <div class="inner" style="color: #000;">
                            @php
                                $valor_saidas_get = \App\Models\MRAFFluxoCaixa::Get_ValorTotal_Saidas();
        ...
        <div class="box-header" style="background-color: #fff; padding-top: 30px">
            <form action="{{ URL('/') }}/mra_fluxo_financeiro/mra_f_fluxo_caixa/filter" method="POST">
                ...
                <div class="row">
                    ...
                    <div size="3" class="inputbox col-md-3">
                        <div class="form-group">
                            ...
                            {!! Form::select('tipo', \App\Models\MRAFFluxoCaixa::Get_options_tipos(), (Request::get('tipo')?Request::get('tipo'):null), ['class' => 'form-control select_single_no_trigger', 'data-live-search' => 'true', "id" => "input_tipo"]) !!}
                        </div>
                    </div>
                    <div size="3" class="inputbox col-md-3">
                        <div class="form-group">
                            {!! Form::label('','Centro de Custo') !!}
                            {!! Form::select('mra_f_centro_custo_id', \App\Models\MRAFCentroCusto::Get_CentroDeCustos_options(), (Request::get('mra_f_centro_custo_id')?Request::get('mra_f_centro_custo_id'):null), ['class' => 'form-control select_single_no_trigger', 'data-live-search' => 'true', "id" => "input_mra_f_centro_custo_id"]) !!}
                        </div>
                    </div>
                    <div size="3" class="inputbox col-md-3">
                        <div class="form-group">
                            {!! Form::label('','Plano de Contas') !!}
                            {!! Form::select('mra_f_plano_contas_id', \App\Models\MRAFPlanoContas::Get_PlanoDeContas_options(), (Request::get('mra_f_plano_contas_id')?Request::get('mra_f_plano_contas_id'):null), ['class' => 'form-control select_single_no_trigger', 'data-live-search' => 'true', "id" => "input_mra_f_plano_contas_id"]) !!}
                        </div>
                    </div>
                    <div size="3" class="inputbox col-md-3">
                        <div class="form-group">
                ...
                <div class="row">
                    <div class="col-md-3 btnsFiltro">
                        <a href="{{ URL('/mra_fluxo_financeiro/mra_f_fluxo_caixa') }}" class="btn btn-default" style="float: left; margin-right: 5px;"><i class="glyphicon glyphicon-trash"></i> Limpar</a>
        ...
        <div class="box">
            ...
            <div class="box-body table-responsive">
                ...
                <table id="<?php echo (!$export_enable_btns ? 'datatable-no-buttons' : 'datatable'); ?>" class="table-st1 display table-striped table-bordered stripe" cellspacing="0" width="100%">
                    ...
                    <tbody>
                        @foreach($MRAFFluxoCaixa as $value)
                            @php
                                $tipo_i     = '';
                                $status_tr  = '';
                                switch($value->tipo){
                            ...
                            <tr class="{{$status_tr}}">
                                ...
                                @endphp
                                <td>{{$cliente_fornecedor}}</td>
                                <td>{{(!empty($value->descricao)?$value->descricao:'---')}}</td>
                                <td>{!! $tipo_i . (!empty($value->tipo)?\App\Models\MRAFFluxoCaixa::Get_tipos($value->tipo):'---') !!}</td>
                                <td>{{(!empty($value->valor)?\App\Helper\Helper::H_Decimal_DB_ptBR($value->valor):'0,00')}}</td>
                                <td>{{($value->MRAFCentroCusto?$value->MRAFCentroCusto->nome:'---')}}</td>
                                <td>{{($value->MRAFPlanoContas?$value->MRAFPlanoContas->nome:'---')}}</td>
                                @if($auth_user__actions_enable_btns)
                                    <td>
                                        {{--@if($permissaoUsuario_auth_user__controller_update)
                                            <a style="float: left; margin-right: 5px;" href="{{ URL('/') }}/mra_fluxo_financeiro/mra_f_fluxo_caixa/{{$value->id}}/edit" alt="Editar" title="Editar" class="btn btn-default">
                                                <span class="glyphicon glyphicon-edit"></span>
                                            </a>
                                        @endif--}}
                                        {{--@if(0)
                                            @if($permissaoUsuario_auth_user__controller_copy)
                                                <a style="float: left; margin-right: 5px;" href="{{ URL('/') }}/mra_fluxo_financeiro/mra_f_fluxo_caixa/{{$value->id}}/copy" alt="Duplicar linha" title="Duplicar linha" class="btn btn-default" onclick="return confirm('Tem certeza que quer duplicar a linha?')" >
                                                    <span class="glyphicon glyphicon-copy"></span>
                                                </a>
                                            @endif
                                        @endif--}}
                                        {{--@if($permissaoUsuario_auth_user__controller_show)
                                            <a style="float: left; margin-right: 5px;" href="{{ URL('/') }}/mra_fluxo_financeiro/mra_f_fluxo_caixa/{{$value->id}}" alt="Visualizar" title="Visualizar" class="btn btn-default">
                                                <span class="glyphicon glyphicon-eye-open"></span>
                                            </a>
                                        @endif--}}
                                        @if($permissaoUsuario_auth_user__controller_destroy)
                                            <form style="float: left; margin-right: 5px;" method="POST" action="{{ route('mra_f_fluxo_caixa.destroy', $value->id) }}" accept-charset="UTF-8">
                                                {!! csrf_field() !!}
                                                {!! method_field('DELETE') !!}
                ...
                <div class="form-group form-group-btn-index">
                    ...
                    {{--@if(\App\Models\Permissions::permissaoUsuario(\Auth::user(), "$controller@store"))
                        <a href="{{ URL('/') }}/mra_fluxo_financeiro/mra_f_fluxo_caixa/create" class="btn btn-default right form-group-btn-index-cadastrar" style="margin-left:15px;"><i class="glyphicon glyphicon-plus"></i> Cadastrar</a>
                    @endif--}}
