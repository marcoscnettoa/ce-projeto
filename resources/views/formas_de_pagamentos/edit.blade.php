...
<section class="content-header Formas de Pagamentos_edit">
...
<section class="content Formas de Pagamentos_edit">
...
<div class="box">
    ...
    @if(\App\Models\Permissions::permissaoUsuario(\Auth::user(), "$controller@destroy"))
        <div class="row" style="position: absolute; right: 0; padding: 5px;">
            <div class="col-md-12">
                <form id="form-destroy" method="POST" action="{{ route('formas_de_pagamentos.destroy', $formas_de_pagamentos->id) }}" accept-charset="UTF-8">
                    {!! csrf_field() !!}
                    {!! method_field('DELETE') !!}
    ...
    {!! Form::open(['url' => "formas_de_pagamentos/$formas_de_pagamentos->id", 'method' => 'put', 'novalidate'=> true, 'enctype' => 'multipart/form-data', 'accept-charset' => 'utf-8', 'id' => 'form_edit_formas_de_pagamentos']) !!}

        @if(\Request::get('modal'))
            {!! Form::hidden('modal-close', 1) !!}
        @endif
        {!! Form::hidden('id', $formas_de_pagamentos->id) !!}

        <div class="box-body" id="div_formas_de_pagamentos">

            <div size="12" class="inputbox col-md-12">
                <div class="form-group">
                    {!! Form::label('','Forma de pagamento') !!}
                    {!! Form::text('forma_de_pagamento', $formas_de_pagamentos->forma_de_pagamento, ['class' => 'form-control' , "id" => "input_forma_de_pagamento"]) !!}
                </div>
            </div>
                ...
                @if(\App\Models\Permissions::permissaoModerador(\Auth::user()))
                    <div class="col-md-12">
                        <div class="form-group">
                            ...
                            {!! Form::select('r_auth', $parserList, $formas_de_pagamentos->r_auth, ['class' => 'form-control']) !!}
                        </div>
                    </div>
                @endif
