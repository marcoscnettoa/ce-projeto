@php
    ...
    /*foreach($formas_de_pagamentos as $val) {
...
<section class="content-header Formas de Pagamentos_index">
...
<section class="content">
    ...
    @if($exibe_filtros)
        <div class="box-header" style="background-color: #fff; padding-top: 15px">
            <form action="{{ URL('/') }}/formas_de_pagamentos/filter" class="form_filter form_filter_formas_de_pagamentos" method="POST">
...
<div class="form-group">
                    ...
                    {!! Form::text('forma_de_pagamento', (!empty(\Request::post('forma_de_pagamento'))?\Request::post('forma_de_pagamento'):null), ['class' => 'form-control' ]) !!}
</div>
</div>

                    <div class="col-md-12 btnsFiltro">
                        <a href="{{ URL('/formas_de_pagamentos') }}" class="btn btn-xs btn-default" style="float: left; margin-right: 5px;"><i class="glyphicon glyphicon-trash"></i> Limpar</a>
            ...
            <div class="box-body">
                @if(($import_enable_btns || $export_enable_btns) && !$isPublic && !$enable_kanban)
                    @include('import', [
                        ...
                        'lote_count'                  => $formas_de_pagamentos_count
                    ])
                @endif
                ...
                @endphp
                             <td style="width:100px; white-space: nowrap;">
                                ...
                                @if($permissaoUsuario_auth_user__controller_update)
                                    <a style="float:none;" href="{{ URL('/') }}/formas_de_pagamentos/{{$value->id}}/edit" alt="Editar" title="Editar" class="btn btn-xs btn-default">
                                ...
                                @if(1)
                                    @if($permissaoUsuario_auth_user__controller_copy)
                                        <a style="float:none;" href="{{ URL('/') }}/formas_de_pagamentos/{{$value->id}}/copy" alt="Duplicar linha" title="Duplicar linha" class="btn btn-xs btn-default" onclick="return confirm('Tem certeza que quer duplicar a linha?')" >
                                ...
                                @if($permissaoUsuario_auth_user__controller_show)
                                    <a style="float:none;" href="{{ URL('/') }}/formas_de_pagamentos/{{$value->id}}" alt="Visualizar" title="Visualizar" class="btn btn-xs btn-default">
                                        <span class="glyphicon glyphicon-eye-open"></span>
                                    </a>
                                @endif

                                @if($permissaoUsuario_auth_user__controller_destroy)
                                    <form style="display:inline-block;" method="POST" action="{{ route('formas_de_pagamentos.destroy', $value->id) }}" accept-charset="UTF-8">
                                        {!! csrf_field() !!}
                                        {!! method_field('DELETE') !!}
                ...
                <table id="datatable-no-buttons" class="display table-striped table-bordered stripe" cellspacing="0" width="100%">
                    <thead>
                        <tr>
                            ...
                            <th class="formas_de_pagamentos_td_forma_de_pagamento" style="white-space: nowrap;">Forma de pagamento</th>
                    ...
                    <tbody>
                        @foreach($formas_de_pagamentos as $value)
                            ...
                            <tr>
                                ...
                                <td style=' white-space: nowrap; '  class='formas_de_pagamentos_td_forma_de_pagamento'>{{$value->forma_de_pagamento}}</td>
                ...
                <div class="form-group form-group-btn-index">
                    ...
                    @if(\App\Models\Permissions::permissaoUsuario(\Auth::user(), "$controller@store") OR $isPublic)
                        <a href="{{ URL('/') }}/formas_de_pagamentos/create" class="btn btn-xs btn-default right form-group-btn-index-cadastrar"><i class="glyphicon glyphicon-plus"></i> Cadastrar</a>
                    @endif
                </div>
            </div>
        </div>
    @else
        @include('formas_de_pagamentos.kanban', [
