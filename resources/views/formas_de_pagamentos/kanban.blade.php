@php
    ...
    $td_colunas_fields      = json_decode('[{"name":"Forma de pagamento","type":"text","actions_advanced":true,"exibeFiltros":true,"className":"FormaDePagamento","cleanName":"forma_de_pagamento"}]',true);
    ...
    foreach($formas_de_pagamentos as $key => $list){
        ...
        $quadros[$list->$kanban_field]['item'][$key]['title']     .= '
            <div class="kanban-buttons text-right">
                <button modulo="formas_de_pagamentos" modulo_id="'.$list->id.'" modulo_method="GET" modulo_url="formas_de_pagamentos/'.$list->id.'/edit" modulo_redirect="true" class="btn btn-xs btn-default kanban-btn-edit"><i class="glyphicon glyphicon-edit" title="Editar"></i></button>
                <button modulo="formas_de_pagamentos" modulo_id="'.$list->id.'" modulo_method="GET" modulo_url="formas_de_pagamentos/'.$list->id.'/copy" modulo_redirect="true" class="btn btn-xs btn-default kanban-btn-copy"><i class="glyphicon glyphicon-copy" title="Duplicar linha"></i></button>
                <button modulo="formas_de_pagamentos" modulo_id="'.$list->id.'" modulo_method="GET" modulo_url="formas_de_pagamentos/'.$list->id.'" modulo_redirect="true" class="btn btn-xs btn-default kanban-btn-view"><i class="glyphicon glyphicon-eye-open" title="Visualizar"></i></button>
                <button modulo="formas_de_pagamentos" modulo_id="'.$list->id.'" class="btn btn-xs btn-danger excluir-auto-confirma" style="float:none;margin: 0px;" title="Excluir"><i class="glyphicon glyphicon-trash"></i></button>
            </div>
        ';
    }
...
<div class="form-group form-group-btn-index text-right">
    @if(\App\Models\Permissions::permissaoUsuario(\Auth::user(), "$controller@store") OR $isPublic)
        <a href="{{ URL('/') }}/formas_de_pagamentos/create" class="btn btn-default form-group-btn-index-cadastrar"><i class="glyphicon glyphicon-plus"></i> Cadastrar</a>
    @endif
</div>
        ...
        var kanban1 = new jKanban({
            ...
            dropEl           : function (el, target, source, sibling) {
                ...
                $.ajax({
                    ...
                    type: 'POST',
                    async: false,
                    url: base+'/formas_de_pagamentos/'+item_id,
                    data: data,
                    success: function(d){
                        if(d.ok != undefined){
                            ajax_return = true;
