...
<section class="content-header Grid Pagamentos_edit">
...
<section class="content Grid Pagamentos_edit">
...
<div class="box">
    ...
    @if(\App\Models\Permissions::permissaoUsuario(\Auth::user(), "$controller@destroy"))
        <div class="row" style="position: absolute; right: 0; padding: 5px;">
            <div class="col-md-12">
                <form id="form-destroy" method="POST" action="{{ route('grid_pagamentos.destroy', $grid_pagamentos->id) }}" accept-charset="UTF-8">
                    {!! csrf_field() !!}
                    {!! method_field('DELETE') !!}
    ...
    {!! Form::open(['url' => "grid_pagamentos/$grid_pagamentos->id", 'method' => 'put', 'novalidate'=> true, 'enctype' => 'multipart/form-data', 'accept-charset' => 'utf-8', 'id' => 'form_edit_grid_pagamentos']) !!}

        @if(\Request::get('modal'))
            {!! Form::hidden('modal-close', 1) !!}
        @endif
        {!! Form::hidden('id', $grid_pagamentos->id) !!}

        <div class="box-body" id="div_grid_pagamentos">

            <div size="3" class="inputbox col-md-3">
                <div class="form-group">
                    {!! Form::label('','Forma de Pagamento') !!}
                    {!! Form::select('forma_de_pagamento', $formas_de_pagamentos_forma_de_pa, $grid_pagamentos->forma_de_pagamento, ['class' => 'form-control  select_relationship ', 'data-sub' => '', 'data-live-search' => 'true',  'data-none-selected-text' => true, 'controller' => 'formas_de_pagamentos' , "id" => "input_forma_de_pagamento"]) !!}
                </div>
            </div>
            ...
            <div size="2" class="inputbox col-md-2">
                <div class="form-group">
                    ...
                    {!! Form::text('valor_pago', $grid_pagamentos->valor_pago, ['class' => 'form-control money' , "id" => "input_valor_pago"]) !!}
                </div>
            </div>
                ...
                @if(\App\Models\Permissions::permissaoModerador(\Auth::user()))
                    <div class="col-md-12">
                        <div class="form-group">
                            ...
                            {!! Form::select('r_auth', $parserList, $grid_pagamentos->r_auth, ['class' => 'form-control']) !!}
                        </div>
                    </div>
                @endif
