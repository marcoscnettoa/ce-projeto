@php
    ...
    $export_options_size        = 'null'; // # -
    $export_options_size_height = null; // # -
    $export_options_size_width  = null; // # -
    $actions_enable_btns        = 1;

    //$kanban_list = array();

    /*foreach($r_clientes_documentos as $val) {
...
<section class="content-header r_clientes_documentos_index">
...
<section class="content">
    ...
    @if($exibe_filtros)
        <div class="box-header" style="background-color: #fff; padding-top: 15px">
            <form action="{{ URL('/') }}/r_clientes_documentos/filter" class="form_filter form_filter_r_clientes_documentos" method="POST">
                <input type="hidden" name="_token" value="{{ csrf_token() }}">
                <div class="row">

                    <div class="col-md-12 btnsFiltro">
                        <a href="{{ URL('/r_clientes_documentos') }}" class="btn btn-xs btn-default" style="float: left; margin-right: 5px;"><i class="glyphicon glyphicon-trash"></i> Limpar</a>
            ...
            <div class="box-body">
                @if(($import_enable_btns || $export_enable_btns) && !$isPublic && !$enable_kanban)
                    @include('import', [
                        ...
                        'lote_count'                  => $r_clientes_documentos_count
                    ])
                @endif
                <div class="table-responsive">
                @php
                    $defaultPositionActions = 'right';
                ...
                @endphp
                             <td style="width:100px; white-space: nowrap;">
                                ...
                                @if($permissaoUsuario_auth_user__controller_update)
                                    <a style="float:none;" href="{{ URL('/') }}/r_clientes_documentos/{{$value->id}}/edit" alt="Editar" title="Editar" class="btn btn-xs btn-default">
                                ...
                                @if(1)
                                    @if($permissaoUsuario_auth_user__controller_copy)
                                        <a style="float:none;" href="{{ URL('/') }}/r_clientes_documentos/{{$value->id}}/copy" alt="Duplicar linha" title="Duplicar linha" class="btn btn-xs btn-default" onclick="return confirm('Tem certeza que quer duplicar a linha?')" >
                                ...
                                @if($permissaoUsuario_auth_user__controller_show)
                                    <a style="float:none;" href="{{ URL('/') }}/r_clientes_documentos/{{$value->id}}" alt="Visualizar" title="Visualizar" class="btn btn-xs btn-default">
                                        <span class="glyphicon glyphicon-eye-open"></span>
                                    </a>
                                @endif

                                @if($permissaoUsuario_auth_user__controller_destroy)
                                    <form style="display:inline-block;" method="POST" action="{{ route('r_clientes_documentos.destroy', $value->id) }}" accept-charset="UTF-8">
                                        {!! csrf_field() !!}
                                        {!! method_field('DELETE') !!}
                ...
                <table id="datatable-no-buttons" class="display table-striped table-bordered stripe" cellspacing="0" width="100%">
                    <thead>
                        <tr>
                            ...
                            <th class="r_clientes_documentos_td_documentos" style="white-space: nowrap;">Documentos</th>
                    ...
                    <tbody>
                        @foreach($r_clientes_documentos as $value)
                            ...
                            <tr>
                                ...
                                <td style=' white-space: nowrap; '  class='r_clientes_documentos_td_documentos'><a href="{{ URL("/") }}/r_clientes_documentos/{{$value->id}}"><i class="glyphicon glyphicon-th-list"></i></a>
</td>
                ...
                <div class="form-group form-group-btn-index">
                    ...
                    @if(\App\Models\Permissions::permissaoUsuario(\Auth::user(), "$controller@store") OR $isPublic)
                        <a href="{{ URL('/') }}/r_clientes_documentos/create" class="btn btn-xs btn-default right form-group-btn-index-cadastrar"><i class="glyphicon glyphicon-plus"></i> Cadastrar</a>
                    @endif
                </div>
            </div>
        </div>
    @else
        @include('r_clientes_documentos.kanban', [
