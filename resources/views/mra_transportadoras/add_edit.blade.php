...
@section('content')
    ...
    {!! Form::open(['url' => "mra_nota_fiscal/mra_transportadoras".($acao=='edit'?'/'.$MRANfTransportadoras->id:''), 'method' => ($acao=='edit'?'put':'post'), 'novalidate'=> true, 'enctype' => 'multipart/form-data', 'accept-charset' => 'utf-8', 'id' => ($acao=='edit'?'form_edit':'form_add').'_mra_transportadoras']) !!}
            ...
            @if($acao=='edit')
                ...
                @if(\App\Models\Permissions::permissaoUsuario(\Auth::user(), "$controller@destroy"))
                    <div class="row" style="position: absolute; right: 0; padding: 5px; z-index: 100;">
                        <div class="col-md-12">
                            <a href="javascript:void(0);" type="button" class="btn btn-danger excluir-auto-confirma" modulo="mra_nota_fiscal/mra_transportadoras" modulo_id="{{$MRANfTransportadoras->id}}"><i class="glyphicon glyphicon-trash"></i></a>
                        </div>
                    </div>
                @endif
            @endif
            <div class="box-body" id="div_mra_transportadoras">
