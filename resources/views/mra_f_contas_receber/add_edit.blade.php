@php
    $acao                     = ((isset($MRAFContasReceber) and !is_null($MRAFContasReceber))?'edit':'add');
    $isPublic                 = 0;
...
@section('content')
    ...
    {!! Form::open(['url' => "mra_fluxo_financeiro/mra_f_contas_receber".($acao=='edit'?'/'.$MRAFContasReceber->id:''), 'method' => ($acao=='edit'?'put':'post'), 'novalidate'=> true, 'enctype' => 'multipart/form-data', 'accept-charset' => 'utf-8', 'id' => ($acao=='edit'?'form_edit':'form_add').'_mra_f_contas_receber']) !!}
    <section class="content" style="min-height: auto;">
        <div class="box" style="margin-bottom: 0; margin-top: 0;">
            @if($acao=='edit')
                {!! Form::hidden('id', $MRAFContasReceber->id) !!}
                @if(\App\Models\Permissions::permissaoUsuario(\Auth::user(), "$controller@destroy"))
                    @if(empty($MRAFContasReceber->notazz_id_documento))
                        <div class="row" style="position: absolute; right: 0; padding: 5px; z-index: 100;">
                            <div class="col-md-12">
                                <a href="javascript:void(0);" type="button" class="btn btn-danger excluir-auto-confirma" modulo="mra_fluxo_financeiro/mra_f_contas_receber" modulo_id="{{$MRAFContasReceber->id}}"><i class="glyphicon glyphicon-trash"></i></a>
                            </div>
                        </div>
                    @endif
                    {{--<div class="box-header">
                        <div class="col-md-12">
                            @php
                                ...
                                switch($MRAFContasReceber->notazz_status){
                                ...
                                $notazz_status  = (!empty($MRAFContasReceber->notazz_status)?\App\Http\Controllers\MRA\MRANotasFiscais::Get_nf_status_nota_fiscal($MRAFContasReceber->notazz_status):'---');
                                // ! Caso o Status foi Cancelada + Cancelamento Forçado
                                if($MRAFContasReceber->notazz_status == 'Cancelada' and $MRAFContasReceber->notazz_status_forcado){
            ...
            <div class="box-body" style="margin-top: 0px;">
                <div size="12" class="inputbox col-md-12">
                    <h2 class="page-header" style="font-size:20px;">
                        <i class="glyphicon glyphicon-th-large"></i> Conta a Receber
                    </h2>
                </div>
                <div size="12" class="inputbox col-md-12">
                    <div class="row">
                        <div size="3" class="inputbox col-md-3">
                            <div class="form-group">
                                ...
                                {!! Form::select('status', \App\Http\Controllers\MRA\MRAListas::Get_options_status_conclusao([""]), ($acao=='edit'?$MRAFContasReceber->status:2), ['class' => 'form-control select_single_no_trigger', 'data-live-search' => 'true', "id" => "input_status"]) !!}
                            </div>
                        </div>
                        <div size="3" class="inputbox col-md-3">
                            <div class="form-group">
                                ...
                                <div class="input-group">
                                    ...
                                    {!! Form::text('data_competencia', ($acao=='edit'?\App\Helper\Helper::H_Data_DB_ptBR($MRAFContasReceber->data_competencia):date('d/m/Y')), ['autocomplete' =>'off', 'class' => 'form-control componenteData_v2', "placeholder"=>"__/__/____", "id" => "input_data_competencia"]) !!}
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="row">
                        <div size="6" class="inputbox col-md-6">
                            <div class="form-group">
                                {!! Form::label('','Cliente') !!} <span style="color: #ff0500;">*</span>
                                {!! Form::select('mra_f_clientes_id', \App\Models\MRAFClientes::lista_clientes(), ($acao=='edit'?$MRAFContasReceber->mra_f_clientes_id:null), ['class' => 'form-control select_single_no_trigger', 'data-live-search' => 'true', "id" => "input_mra_f_clientes_id"]) !!}
                            </div>
                        </div>
                    </div>
                    <div class="row">
                        <div size="4" class="inputbox col-md-4">
                            <div class="form-group ss-st2">
                                ...
                                {!! Form::select('mra_f_centro_custo_id', \App\Models\MRAFCentroCusto::Get_CentroDeCustos_options(), ($acao=='edit'?$MRAFContasReceber->mra_f_centro_custo_id:null), ['class' => 'form-control select_single_no_trigger', 'data-live-search' => 'true', "id" => "input_mra_f_centro_custo_id"]) !!}
                            </div>
                        </div>
                        <div size="4" class="inputbox col-md-4">
                            <div class="form-group">
                                {!! Form::label('','Plano de Conta') !!}
                                {!! Form::select('mra_f_plano_contas_id', \App\Models\MRAFPlanoContas::Get_PlanoDeContas_options(), ($acao=='edit'?$MRAFContasReceber->mra_f_plano_contas_id:null), ['class' => 'form-control select_single_no_trigger', 'data-live-search' => 'true', "id" => "input_mra_f_plano_contas_id"]) !!}
                            </div>
                        </div>
                        <div size="4" class="inputbox col-md-4">
                            <div class="form-group">
                                {!! Form::label('','Conta Recebimento') !!} <span style="color: #ff0500;">*</span>
                                {!! Form::select('mra_f_contas_bancarias_id', \App\Models\MRAFTransferenciaContas::Get_ContasBancarias_options(), ($acao=='edit'?$MRAFContasReceber->mra_f_contas_bancarias_id:null), ['class' => 'form-control select_single_no_trigger', 'data-live-search' => 'true', "id" => "input_mra_f_contas_bancarias_id"]) !!}
                            </div>
                        </div>
                    </div>
                    <div class="row">
                        <div size="12" class="inputbox col-md-12">
                            <div class="form-group">
                                ...
                                {!! Form::text('descricao', ($acao=='edit'?$MRAFContasReceber->descricao:null), ['class' => 'form-control', "id" => "input_descricao", "maxlength"=>200]) !!}
                            </div>
                        </div>
                    </div>
                    <div class="row">
                        <div size="6" class="inputbox col-md-6">
                            ...
                            @if($acao == 'edit' and $MRAFContasReceber->anexo)
                                {!! Form::hidden('anexo', $MRAFContasReceber->anexo) !!}
                                @if($MRAFContasReceber->anexo && count(explode(".", $MRAFContasReceber->anexo)) >= 2)
                                    <a class="fancybox" rel="gallery1" target="_blank" href="{{ $fileurlbase . "images/" . $MRAFContasReceber->anexo }}">
                                        <img src="{{in_array(explode(".", $MRAFContasReceber->anexo)[1], array("mp4", "pdf", "doc", "docx", "rar", "zip", "txt", "7zip", "csv", "xls", "xlsx")) ? URL("/") . "/" . explode(".", $MRAFContasReceber->anexo)[1] . "-icon.png" : $fileurlbase . "images/" . $MRAFContasReceber->anexo}}" height="50">
                                    </a>
                                @endif
                            @endif
                        ...
                        <div size="6" class="inputbox col-md-6">
                            ...
                            @if($acao == 'edit' and $MRAFContasReceber->anexo2)
                                {!! Form::hidden('anexo2', $MRAFContasReceber->anexo2) !!}
                                @if($MRAFContasReceber->anexo2 && count(explode(".", $MRAFContasReceber->anexo2)) >= 2)
                                    <a class="fancybox" rel="gallery1" target="_blank" href="{{ $fileurlbase . "images/" . $MRAFContasReceber->anexo2 }}">
                                        <img src="{{in_array(explode(".", $MRAFContasReceber->anexo2)[1], array("mp4", "pdf", "doc", "docx", "rar", "zip", "txt", "7zip", "csv", "xls", "xlsx")) ? URL("/") . "/" . explode(".", $MRAFContasReceber->anexo2)[1] . "-icon.png" : $fileurlbase . "images/" . $MRAFContasReceber->anexo2}}" height="50">
                                    </a>
                                @endif
                            @endif
                            <i class='glyphicon glyphicon-trash input_remove' style='margin-top: 5px;'></i>
                ...
                <div size="12" class="inputbox col-md-12">
                    <div class="row">
                        <div size="4" class="inputbox col-md-4">
                            <div class="form-group">
                                ...
                                {!! Form::select('tipo_pagamento', \App\Http\Controllers\MRA\MRAListas::Get_options_tipo_pagamento(), ($acao=='edit'?$MRAFContasReceber->tipo_pagamento:null), ['class' => 'form-control select_single_no_trigger', 'data-live-search' => 'true', "id" => "input_tipo_pagamento"]) !!}
                            </div>
                        </div>
                        <div size="4" class="inputbox col-md-4">
                            <div class="form-group">
                                ...
                                <div class="input-group">
                                    <div class="input-group-addon"><i class="fa fa-calendar"></i></div>
                                    {!! Form::text('vencimento', ($acao=='edit'?\App\Helper\Helper::H_Data_DB_ptBR($MRAFContasReceber->vencimento):null), ['autocomplete' =>'off', 'class' => 'form-control componenteData_v2', "placeholder"=>"__/__/____", "id" => "input_vencimento"]) !!}
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div size="3" class="inputbox col-md-3">
                            <div class="form-group">
                                ...
                                <div class="input-group">
                                    ...
                                    {!! Form::text('valor', ($acao=='edit'?\App\Helper\Helper::H_Decimal_DB_ptBR($MRAFContasReceber->valor):null), ['class' => 'form-control money_v2', "placeholder" => "0,00", "id" => "input_valor"]) !!}
                                </div>
                            </div>
                        </div>
                        <div size="3" class="inputbox col-md-3">
                            <div class="form-group">
                                ...
                                <div class="input-group">
                                    <div class="input-group-addon"><strong>R$</strong></div>
                                    {!! Form::text('juros', ($acao=='edit'?\App\Helper\Helper::H_Decimal_DB_ptBR($MRAFContasReceber->juros):null), ['class' => 'form-control money_v2', "placeholder" => "0,00", "id" => "input_juros"]) !!}
                                </div>
                            </div>
                        </div>
                        <div size="3" class="inputbox col-md-3">
                            <div class="form-group">
                                ...
                                <div class="input-group">
                                    <div class="input-group-addon"><strong>R$</strong></div>
                                    {!! Form::text('multa', ($acao=='edit'?\App\Helper\Helper::H_Decimal_DB_ptBR($MRAFContasReceber->multa):null), ['class' => 'form-control money_v2', "placeholder" => "0,00", "id" => "input_multa"]) !!}
                                </div>
                            </div>
                        </div>
                        @php
                            ...
                            if($acao == 'edit'){
                                $valor_total_a_receber  = ($MRAFContasReceber->valor + $MRAFContasReceber->juros + $MRAFContasReceber->multa);
                            }
                        @endphp
                        <div id="box_valor_total_a_receber" size="3" class="inputbox col-md-3">
                            <div class="form-group">
                                {!! Form::label('','Valor Total a Receber') !!}
                                <div class="input-group">
                                    <div class="input-group-addon"><strong>R$</strong></div>
                                    {!! Form::text('valor_total_a_receber', ($acao=='edit'?number_format($valor_total_a_receber,2,',','.'):null), ['class' => 'form-control', "disabled" => true, "placeholder" => "0,00", "id" => "input_valor_total_a_receber"]) !!}
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="row">
                        <div id="box_av_forma_pagamento" size="4" class="inputbox col-md-4" style="display:none;">
                            <div class="form-group">
                                ...
                                {!! Form::select('av_forma_pagamento', \App\Http\Controllers\MRA\MRAListas::Get_options_formas_pagamentos(), ($acao=='edit'?$MRAFContasReceber->av_forma_pagamento:null), ['class' => 'form-control select_single_no_trigger', 'data-live-search' => 'true', "id" => "input_av_forma_pagamento"]) !!}
                            </div>
                        </div>
                        <div id="box_av_status_pagamento" size="3" class="inputbox col-md-3" style="display:none;">
                            <div class="form-group">
                                ...
                                {!! Form::select('av_status_pagamento', \App\Http\Controllers\MRA\MRAListas::Get_options_status_pagamento([""]), (($acao=='edit' and !empty($MRAFContasReceber->av_status_pagamento))?$MRAFContasReceber->av_status_pagamento:2), ['class' => 'form-control select_single_no_trigger', 'data-live-search' => 'true', "id" => "input_av_status_pagamento"]) !!}
                            </div>
                        </div>
                    </div>
                    <div class="row row_bg_st1">
                        <div size="3" class="inputbox col-md-3">
                            <div class="form-group">
                                ...
                                <div class="input-group">
                                    <div class="input-group-addon"><strong>R$</strong></div>
                                    {!! Form::text('valor_entrada', ($acao=='edit'?\App\Helper\Helper::H_Decimal_DB_ptBR($MRAFContasReceber->valor_entrada):null), ['class' => 'form-control money_v2', "placeholder" => "0,00", "id" => "input_valor_entrada"]) !!}
                                </div>
                            </div>
                        </div>
                        <div size="4" class="inputbox col-md-4">
                            <div class="form-group">
                                ...
                                {!! Form::select('entrada_forma_pagamento', \App\Http\Controllers\MRA\MRAListas::Get_options_formas_pagamentos(), ($acao=='edit'?$MRAFContasReceber->entrada_forma_pagamento:null), ['class' => 'form-control select_single_no_trigger', 'data-live-search' => 'true', "id" => "input_entrada_forma_pagamento"]) !!}
                            </div>
                        </div>
                        <div size="3" class="inputbox col-md-3">
                            <div class="form-group">
                                {!! Form::label('','Status Pagamento Entrada',['class'=>'text-warning']) !!}
                                {!! Form::select('entrada_status_pagamento', \App\Http\Controllers\MRA\MRAListas::Get_options_status_pagamento(), ($acao=='edit'?$MRAFContasReceber->entrada_status_pagamento:null), ['class' => 'form-control select_single_no_trigger', 'data-live-search' => 'true', "id" => "input_entrada_status_pagamento"]) !!}
                            </div>
                        </div>
                    </div>
                    <div class="row">
                        @php
                            ...
                            if($acao == 'edit'){
                                $valor_total_a_parcelar = ($MRAFContasReceber->valor + $MRAFContasReceber->juros + $MRAFContasReceber->multa - $MRAFContasReceber->valor_entrada);
                            }
                        @endphp
                        ...
                        <div id="box_parcelas" size="3" class="inputbox col-md-3" style="display:none;">
                            <div class="form-group">
                                ...
                                <div class="input-group mb_15p">
                                    {!! Form::number('parcelas', ($acao=='edit'?$MRAFContasReceber->parcelas:null), ['class' => 'form-control', "placeholder"=>"0", "id" => "input_parcelas", "min"=>0]) !!}
                    ...
                    <div id="box_parcelas_grid" class="row" style="display:none;">
                        ...
                        <div class="GridContasReceberParcelas_grid Grid_grid" count="0">
                            @php
                                function GridContasReceberParcelas($acao,$MRAFContasReceber,$Item) {
                            @endphp
                            <div class="divdefault item">
                                <div class="col-md-12" style="margin-bottom: 10px;">
                                    <div class="row">
                                        {!! Form::hidden('mra_f_contas_receber_parc_id[]', ($acao=='edit'?($Item?$Item->id:null):null), []) !!}
                                        @php
                                            $parcelas_posicao = null;
                                            if($MRAFContasReceber and $Item){
                                                $parcelas_posicao = ($Item->k+1)."/".$MRAFContasReceber->parcelas;
                                            }
                                        @endphp
                                ...
                                <i class="glyphicon glyphicon-trash text-danger grid_remove_v2" data="GridContasReceberParcelas_grid"></i>
                            </div>
                            @php
                                }
                            @endphp
                            @php
                                $MRAFContasReceber_old = [];
                                if(old('mra_f_contas_receber_parc_id')){
                                    foreach(old('mra_f_contas_receber_parc_id') as $K => $old) {
                                        $old_stdClass                   = new stdClass();
                                        $old_stdClass->k                = $K;
                                        $old_stdClass->id               = old('mra_f_contas_receber_parc_id')[$K];
                                        ...
                                        $MRAFContasReceber_old[]        = $old_stdClass;
                                    }
                                }
                            @endphp
                            @if(count($MRAFContasReceber_old))
                                @foreach($MRAFContasReceber_old as $K => $Item)
                                    {{ GridContasReceberParcelas($acao,$MRAFContasReceber,$Item) }}
                                @endforeach
                            @elseif($MRAFContasReceber and $MRAFContasReceber->MRAFContasReceberParcelas and count($MRAFContasReceber->MRAFContasReceberParcelas))
                                @foreach($MRAFContasReceber->MRAFContasReceberParcelas as $K => $Item)
                                    @php
                                        $Item->k = $K;
                                    @endphp
                                    {{ GridContasReceberParcelas($acao,$MRAFContasReceber,$Item) }}
                                @endforeach
                            @else
                                {{ GridContasReceberParcelas($acao,null,null) }}
                            @endif
                        </div>
                        <div class="col-md-12 text-right">
                            <i class="glyphicon glyphicon-plus multiple_add_v2" data="GridContasReceberParcelas_grid"></i>
                        </div>
                    </div>
                </div>
                {{--@if(0)
    ...
    @section('script')
        <script type="text/javascript">
            ...
            function calculo_valor_total(){
                ...
                let valor_a_receber_calc    = (valor + valor_juros + valor_multa);
                let valor_a_parcelar_calc   = (valor + valor_juros + valor_multa - valor_entrada);

                $("#input_valor_total_a_receber").val(RA.format.Decimal_DB_ptBR(valor_a_receber_calc));
