...
<section class="content-header Trechos_index">
    <h1>Trechos</h1>
</section>
...
<section class="content">
    ...
    @if($exibe_filtros)
        <div class="box-header" style="background-color: #fff; padding-top: 15px">
            <form action="{{ URL('/') }}/trechos/filter" class="form_filter form_filter_trechos" method="POST">
...
<div class="form-group">
                    {!! Form::label('','Trechos') !!}
                    {!! Form::text('trechos', (!empty(\Request::post('trechos'))?\Request::post('trechos'):null), ['class' => 'form-control' ]) !!}
</div>
</div>

                    <div class="col-md-12 btnsFiltro">
                        <a href="{{ URL('/trechos') }}" class="btn btn-xs btn-default" style="float: left; margin-right: 5px;"><i class="glyphicon glyphicon-trash"></i> Limpar</a>
            ...
            <div class="box-body">
                ...
                @endphp
                             <td style="width:100px; white-space: nowrap;">
                                ...
                                @if($permissaoUsuario_auth_user__controller_update)
                                    <a style="float:none;" href="{{ URL('/') }}/trechos/{{$value->id}}/edit" alt="Editar" title="Editar" class="btn btn-xs btn-default">
                                ...
                                @if(1)
                                    @if($permissaoUsuario_auth_user__controller_copy)
                                        <a style="float:none;" href="{{ URL('/') }}/trechos/{{$value->id}}/copy" alt="Duplicar linha" title="Duplicar linha" class="btn btn-xs btn-default" onclick="return confirm('Tem certeza que quer duplicar a linha?')" >
                                ...
                                @if($permissaoUsuario_auth_user__controller_show)
                                    <a style="float:none;" href="{{ URL('/') }}/trechos/{{$value->id}}" alt="Visualizar" title="Visualizar" class="btn btn-xs btn-default">
                                        <span class="glyphicon glyphicon-eye-open"></span>
                                    </a>
                                @endif

                                @if($permissaoUsuario_auth_user__controller_destroy)
                                    <form style="display:inline-block;" method="POST" action="{{ route('trechos.destroy', $value->id) }}" accept-charset="UTF-8">
                                        {!! csrf_field() !!}
                                        {!! method_field('DELETE') !!}
                ...
                <table id="datatable-no-buttons" class="display table-striped table-bordered stripe" cellspacing="0" width="100%">
                    <thead>
                        <tr>
                            ...
                            <th class="trechos_td_trechos" style="white-space: nowrap;">Trechos</th>
                    ...
                    <tbody>
                        @foreach($trechos as $value)
                            ...
                            <tr>
                                ...
                                <td style="text-transform: uppercase;"  class="trechos_td_trechos">{{$value->trechos}}</td>
                ...
                <div class="form-group form-group-btn-index">
                    ...
                    @if(\App\Models\Permissions::permissaoUsuario(\Auth::user(), "$controller@store") OR $isPublic)
                        <a href="{{ URL('/') }}/trechos/create" class="btn btn-xs btn-default right form-group-btn-index-cadastrar"><i class="glyphicon glyphicon-plus"></i> Cadastrar</a>
                    @endif
                </div>
            </div>
        </div>
    @else
        @include('trechos.kanban', [
            'trechos' => $trechos, // # -
