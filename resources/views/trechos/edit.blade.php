...
<section class="content-header Trechos_edit">
    <h1>Trechos </h1>
    <!--@if(!$isPublic)
    <ol class="breadcrumb">
...
<section class="content Trechos_edit">
...
<div class="box">
    ...
    @if(\App\Models\Permissions::permissaoUsuario(\Auth::user(), "$controller@destroy"))
        <div class="row" style="position: absolute; right: 0; padding: 5px;">
            <div class="col-md-12">
                <form id="form-destroy" method="POST" action="{{ route('trechos.destroy', $trechos->id) }}" accept-charset="UTF-8">
                    {!! csrf_field() !!}
                    {!! method_field('DELETE') !!}
    ...
    {!! Form::open(['url' => "trechos/$trechos->id", 'method' => 'put', 'novalidate'=> true, 'enctype' => 'multipart/form-data', 'accept-charset' => 'utf-8', 'id' => 'form_edit_trechos']) !!}

        @if(\Request::get('modal'))
            {!! Form::hidden('modal-close', 1) !!}
        @endif
        {!! Form::hidden('id', $trechos->id) !!}

        <div class="box-body" id="div_trechos">

            <div size="12" class="inputbox col-md-12">
                <div class="form-group">
                    {!! Form::label('','Trechos') !!}
                    {!! Form::text('trechos', $trechos->trechos, ['class' => 'form-control' , "id" => "input_trechos", "style" => "text-transform: uppercase;", "oninput" => "this.value = this.value.toUpperCase();"]) !!}
                </div>
            </div>
                ...
                @if(\App\Models\Permissions::permissaoModerador(\Auth::user()))
                    <div class="col-md-12">
                        <div class="form-group">
                            ...
                            {!! Form::select('r_auth', $parserList, $trechos->r_auth, ['class' => 'form-control']) !!}
                        </div>
                    </div>
                @endif
