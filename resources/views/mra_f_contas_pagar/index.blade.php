    ...
    <section class="content">
            ...
            <div class="row">
                <div class="col-md-3">
                    <div class="small-box v2 bg-success">
                        <div class="inner" style="color: #000;">
                            <h3>{{ \App\Models\MRAFContasPagar::Get_QtStatus_Concluidos() }}</h3>
                ...
                <div class="col-md-3">
                    <div class="small-box v2 bg-warning">
                        <div class="inner" style="color: #000;">
                            <h3>{{ \App\Models\MRAFContasPagar::Get_QtStatus_Pendentes() }}</h3>
                ...
                <div class="col-md-3">
                    <div class="small-box v2 bg-success">
                        <div class="inner" style="color: #000;">
                            @php
                                $valor_total_pagos = \App\Helper\Helper::H_Decimal_DB_ptBR(\App\Models\MRAFContasPagar::Get_ValorTotalPagamentos_Pagos());
                ...
                <div class="col-md-3">
                    <div class="small-box v2 bg-warning">
                        <div class="inner" style="color: #000;">
                            @php
                                $valor_total_pendentes = \App\Helper\Helper::H_Decimal_DB_ptBR(\App\Models\MRAFContasPagar::Get_ValorTotalPagamentos_Pendentes());
        ...
        {{--@if($exibe_filtros)--}}
            <div class="box-header" style="background-color: #fff; padding-top: 30px">
                <form action="{{ URL('/') }}/mra_fluxo_financeiro/mra_f_contas_pagar/filter" method="POST">
                    ...
                    <div class="row">
                        <div class="col-md-3 btnsFiltro">
                            <a href="{{ URL('/mra_fluxo_financeiro/mra_f_contas_pagar') }}" class="btn btn-default" style="float: left; margin-right: 5px;"><i class="glyphicon glyphicon-trash"></i> Limpar</a>
            ...
            <div class="box-body table-responsive">
                <table id="<?php echo (!$export_enable_btns ? 'datatable-no-buttons' : 'datatable'); ?>" class="table-st1 display table-striped table-bordered stripe" cellspacing="0" width="100%">
                    <thead>
                        <tr>
                            ...
                            <th style="white-space: nowrap;">Valor Total a Pagar</th>
                            <th style="white-space: nowrap;">Tipo de Pagamento</th>
                    ...
                    <tbody>
                        @foreach($MRAFContasPagar as $value)
                            @php
                            ...
                            <tr class="{{$status_tr}}" _id="{{$value->id}}">
                                ...
                                <td style="white-space: nowrap;">{{(($value->MRAFFornecedores and !empty($value->MRAFFornecedores->nome))?$value->MRAFFornecedores->nome:'---')}}</td>
                                ...
                                @php
                                    $valor_total_a_pagar  = \App\Helper\Helper::H_Decimal_DB_ptBR($value->valor + $value->juros + $value->multa);
                                    $valor_total_a_pagar  = (!empty($valor_total_a_pagar)?$valor_total_a_pagar:'0,00');
                                @endphp
                                <td style="white-space: nowrap;">{{ $valor_total_a_pagar }}</td>
                                ...
                                @if($auth_user__actions_enable_btns)
                                    <td>
                                        @if($permissaoUsuario_auth_user__controller_update)
                                            <a style="float: left; margin-right: 5px;" href="{{ URL('/') }}/mra_fluxo_financeiro/mra_f_contas_pagar/{{$value->id}}/edit" alt="Editar" title="Editar" class="btn btn-default">
                                                <span class="glyphicon glyphicon-edit"></span>
                                            </a>
                                        @endif
                                        {{--@if(0)
                                            @if($permissaoUsuario_auth_user__controller_copy)
                                                <a style="float: left; margin-right: 5px;" href="{{ URL('/') }}/mra_fluxo_financeiro/mra_f_contas_pagar/{{$value->id}}/copy" alt="Duplicar linha" title="Duplicar linha" class="btn btn-default" onclick="return confirm('Tem certeza que quer duplicar a linha?')" >
                                                    <span class="glyphicon glyphicon-copy"></span>
                                                </a>
                                            @endif
                                        @endif--}}
                                        {{--@if($permissaoUsuario_auth_user__controller_show)
                                            <a style="float: left; margin-right: 5px;" href="{{ URL('/') }}/mra_fluxo_financeiro/mra_f_contas_pagar/{{$value->id}}" alt="Visualizar" title="Visualizar" class="btn btn-default">
                                                <span class="glyphicon glyphicon-eye-open"></span>
                                            </a>
                                        @endif--}}
                                        @if($permissaoUsuario_auth_user__controller_destroy)
                                            <form style="float: left; margin-right: 5px;" method="POST" action="{{ route('mra_f_contas_pagar.destroy', $value->id) }}" accept-charset="UTF-8">
                                                {!! csrf_field() !!}
                                                {!! method_field('DELETE') !!}
                ...
                <div class="form-group form-group-btn-index">
                    ...
                    @if(\App\Models\Permissions::permissaoUsuario(\Auth::user(), "$controller@store"))
                        <a href="{{ URL('/') }}/mra_fluxo_financeiro/mra_f_contas_pagar/create" class="btn btn-default right form-group-btn-index-cadastrar"><i class="glyphicon glyphicon-plus"></i> Cadastrar</a>
                    @endif
                </div>
            </div>
        </div>
