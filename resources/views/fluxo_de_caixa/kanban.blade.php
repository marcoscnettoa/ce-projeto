@php
    ...
    $td_colunas_fields      = json_decode('[{"name":"Saldo Inicial","type":"money","disabled":false,"note":null,"value":null,"class":"money","sum":true,"size":"2","actions_advanced":true,"hidden_view":true,"hidden_add":true,"hidden_edit":true,"buttonGlyphicon":"glyphicon glyphicon-usd","alias":"Saldo do dia","className":"SaldoInicial","cleanName":"saldo_inicial"},{"name":"Movimenta\u00e7\u00e3o","type":"text","disabled":false,"note":null,"value":null,"actions_advanced":true,"exibeFiltros":true,"size":"10","alias":null,"class":"text","className":"Movimentacao","cleanName":"movimentacao"},{"name":"Recebimentos","type":"section","disabled":false,"note":null,"value":null,"className":"Recebimentos","cleanName":"recebimentos"},{"name":"Data do Recebimento","type":"data","size":"2","disabled":false,"note":null,"value":null,"class":"data","actions_advanced":true,"exibeFiltros":true,"operatorfield":false,"hidden_view":true,"hidden_add":true,"hidden_edit":true,"buttonGlyphicon":"glyphicon glyphicon-calendar","className":"DataDoRecebimento","cleanName":"data_do_recebimento"},{"name":"Valor Recebido","type":"money","disabled":false,"note":null,"value":null,"class":"money","sum":true,"size":"3","buttonGlyphicon":"glyphicon glyphicon-usd","actions_advanced":true,"exibeFiltros":true,"operatorfield":false,"className":"ValorRecebido","cleanName":"valor_recebido"},{"name":"Pagamentos","type":"section","disabled":false,"note":null,"value":null,"className":"Pagamentos","cleanName":"pagamentos"},{"name":"Data do Pagamento","type":"data","disabled":false,"note":null,"value":null,"size":"2","actions_advanced":true,"exibeFiltros":true,"hidden_edit":true,"hidden_add":true,"hidden_view":true,"operatorfield":false,"buttonGlyphicon":"glyphicon glyphicon-calendar","class":"data","className":"DataDoPagamento","cleanName":"data_do_pagamento"},{"name":"Total a Pagar","type":"money","size":"3","disabled":false,"note":null,"value":null,"class":"money","sum":true,"buttonGlyphicon":"glyphicon glyphicon-usd","actions_advanced":true,"exibeFiltros":true,"operatorfield":false,"className":"TotalAPagar","cleanName":"total_a_pagar"},{"name":"Total","type":"section","disabled":false,"note":null,"value":null,"className":"Total","cleanName":"total"},{"name":"Data Atual","type":"data","disabled":true,"note":null,"value":null,"class":"data","size":"4","buttonGlyphicon":"glyphicon glyphicon-calendar","actions_advanced":true,"hidden_edit":true,"hidden_add":true,"exibeFiltros":true,"operatorfield":false,"aliasBetween":"&","className":"DataAtual","cleanName":"data_atual"},{"name":"Ghost camp","type":"calculo","size":"4","actions_advanced":true,"alias":"Calculando","disabled":true,"note":null,"value":null,"class":"calculo","dsfooter":"Saldo Final  ->","hidden_edit":true,"hidden_add":true,"className":"GhostCamp","cleanName":"ghost_camp"},{"name":"Saldo da Transa\u00e7\u00e3o","type":"calculo","size":"4","actions_advanced":true,"hidden_edit":true,"hidden_add":true,"disabled":false,"note":null,"value":null,"class":"calculo","expression":"COL4 - COL7","decimals":2,"sum":true,"className":"SaldoDaTransacao","cleanName":"saldo_da_transacao"}]',true);
    ...
    foreach($fluxo_de_caixa as $key => $list){
        ...
        $quadros[$list->$kanban_field]['item'][$key]['title']     .= '
            <div class="kanban-buttons text-right">
                <button modulo="fluxo_de_caixa" modulo_id="'.$list->id.'" modulo_method="GET" modulo_url="fluxo_de_caixa/'.$list->id.'/edit" modulo_redirect="true" class="btn btn-xs btn-default kanban-btn-edit"><i class="glyphicon glyphicon-edit" title="Editar"></i></button>
                <button modulo="fluxo_de_caixa" modulo_id="'.$list->id.'" modulo_method="GET" modulo_url="fluxo_de_caixa/'.$list->id.'/copy" modulo_redirect="true" class="btn btn-xs btn-default kanban-btn-copy"><i class="glyphicon glyphicon-copy" title="Duplicar linha"></i></button>
                <button modulo="fluxo_de_caixa" modulo_id="'.$list->id.'" modulo_method="GET" modulo_url="fluxo_de_caixa/'.$list->id.'" modulo_redirect="true" class="btn btn-xs btn-default kanban-btn-view"><i class="glyphicon glyphicon-eye-open" title="Visualizar"></i></button>
                <button modulo="fluxo_de_caixa" modulo_id="'.$list->id.'" class="btn btn-xs btn-danger excluir-auto-confirma" style="float:none;margin: 0px;" title="Excluir"><i class="glyphicon glyphicon-trash"></i></button>
            </div>
        ';
    }
...
<div class="form-group form-group-btn-index text-right">
    @if(\App\Models\Permissions::permissaoUsuario(\Auth::user(), "$controller@store") OR $isPublic)
        <a href="{{ URL('/') }}/fluxo_de_caixa/create" class="btn btn-default form-group-btn-index-cadastrar"><i class="glyphicon glyphicon-plus"></i> Cadastrar</a>
    @endif
</div>
        ...
        var kanban1 = new jKanban({
            ...
            dropEl           : function (el, target, source, sibling) {
                ...
                $.ajax({
                    ...
                    type: 'POST',
                    async: false,
                    url: base+'/fluxo_de_caixa/'+item_id,
                    data: data,
                    success: function(d){
                        if(d.ok != undefined){
                            ajax_return = true;
