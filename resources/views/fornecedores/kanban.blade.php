@php
    ...
    $td_colunas_fields      = json_decode('[{"name":"Fornecedor","type":"text","actions_advanced":true,"exibeFiltros":true,"inputToLower":true,"className":"Fornecedor","cleanName":"fornecedor"}]',true);
    ...
    foreach($fornecedores as $key => $list){
        ...
        $quadros[$list->$kanban_field]['item'][$key]['title']     .= '
            <div class="kanban-buttons text-right">
                <button modulo="fornecedores" modulo_id="'.$list->id.'" modulo_method="GET" modulo_url="fornecedores/'.$list->id.'/edit" modulo_redirect="true" class="btn btn-xs btn-default kanban-btn-edit"><i class="glyphicon glyphicon-edit" title="Editar"></i></button>
                <button modulo="fornecedores" modulo_id="'.$list->id.'" modulo_method="GET" modulo_url="fornecedores/'.$list->id.'/copy" modulo_redirect="true" class="btn btn-xs btn-default kanban-btn-copy"><i class="glyphicon glyphicon-copy" title="Duplicar linha"></i></button>
                <button modulo="fornecedores" modulo_id="'.$list->id.'" modulo_method="GET" modulo_url="fornecedores/'.$list->id.'" modulo_redirect="true" class="btn btn-xs btn-default kanban-btn-view"><i class="glyphicon glyphicon-eye-open" title="Visualizar"></i></button>
                <button modulo="fornecedores" modulo_id="'.$list->id.'" class="btn btn-xs btn-danger excluir-auto-confirma" style="float:none;margin: 0px;" title="Excluir"><i class="glyphicon glyphicon-trash"></i></button>
            </div>
        ';
    }
...
<div class="form-group form-group-btn-index text-right">
    @if(\App\Models\Permissions::permissaoUsuario(\Auth::user(), "$controller@store") OR $isPublic)
        <a href="{{ URL('/') }}/fornecedores/create" class="btn btn-default form-group-btn-index-cadastrar"><i class="glyphicon glyphicon-plus"></i> Cadastrar</a>
    @endif
</div>
