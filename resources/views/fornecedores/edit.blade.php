...
<section class="content-header Fornecedores_edit">
    <h1>Fornecedores </h1>
    <!--@if(!$isPublic)
    <ol class="breadcrumb">
        <li><a href="{{ URL('/') }}">Home</a></li>
        <li><a href="{{ URL('/') }}/fornecedores">Fornecedores</a></li>
        <li class="active">#{{$fornecedores->id}}</li>
    </ol>
    @endif-->
</section>

<section class="content Fornecedores_edit">
...
<div class="box">
    ...
    @if(\App\Models\Permissions::permissaoUsuario(\Auth::user(), "$controller@destroy"))
        <div class="row" style="position: absolute; right: 0; padding: 5px;">
            <div class="col-md-12">
                <form id="form-destroy" method="POST" action="{{ route('fornecedores.destroy', $fornecedores->id) }}" accept-charset="UTF-8">
                    {!! csrf_field() !!}
                    {!! method_field('DELETE') !!}
    ...
    {!! Form::open(['url' => "fornecedores/$fornecedores->id", 'method' => 'put', 'novalidate'=> true, 'enctype' => 'multipart/form-data', 'accept-charset' => 'utf-8', 'id' => 'form_edit_fornecedores']) !!}

        @if(\Request::get('modal'))
            {!! Form::hidden('modal-close', 1) !!}
        @endif
        {!! Form::hidden('id', $fornecedores->id) !!}
        ...
        <div class="box-body" id="div_fornecedores">
            ...
            <div size="12" class="inputbox col-md-12">
                <div class="form-group">
                    ...
                    {!! Form::text('fornecedor', $fornecedores->fornecedor, ['class' => 'form-control' , "id" => "input_fornecedor", "style" => "text-transform: uppercase;", "oninput" => "this.value = this.value.toUpperCase();"]) !!}
                </div>
            </div>
                ...
                @if(\App\Models\Permissions::permissaoModerador(\Auth::user()))
                    <div class="col-md-12">
                        <div class="form-group">
                            ...
                            {!! Form::select('r_auth', $parserList, $fornecedores->r_auth, ['class' => 'form-control']) !!}
                        </div>
                    </div>
                @endif
