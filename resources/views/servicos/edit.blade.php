...
<section class="content-header Serviços_edit">
    <h1>Serviços </h1>
    <!--@if(!$isPublic)
    <ol class="breadcrumb">
        <li><a href="{{ URL('/') }}">Home</a></li>
        <li><a href="{{ URL('/') }}/servicos">Serviços</a></li>
        <li class="active">#{{$servicos->id}}</li>
    </ol>
    @endif-->
</section>

<section class="content Serviços_edit">
...
<div class="box">
    ...
    @if(\App\Models\Permissions::permissaoUsuario(\Auth::user(), "$controller@destroy"))
        <div class="row" style="position: absolute; right: 0; padding: 5px;">
            <div class="col-md-12">
                <form id="form-destroy" method="POST" action="{{ route('servicos.destroy', $servicos->id) }}" accept-charset="UTF-8">
                    {!! csrf_field() !!}
                    {!! method_field('DELETE') !!}
    ...
    {!! Form::open(['url' => "servicos/$servicos->id", 'method' => 'put', 'novalidate'=> true, 'enctype' => 'multipart/form-data', 'accept-charset' => 'utf-8', 'id' => 'form_edit_servicos']) !!}

        @if(\Request::get('modal'))
            {!! Form::hidden('modal-close', 1) !!}
        @endif
        {!! Form::hidden('id', $servicos->id) !!}

        <div class="box-body" id="div_servicos">

            <div size="12" class="inputbox col-md-12">
                <div class="form-group">
                    {!! Form::label('','Serviço') !!}
                    {!! Form::text('servico', $servicos->servico, ['class' => 'form-control' , "id" => "input_servico", "style" => "text-transform: uppercase;", "oninput" => "this.value = this.value.toUpperCase();"]) !!}
                </div>
            </div>
                ...
                @if(\App\Models\Permissions::permissaoModerador(\Auth::user()))
                    <div class="col-md-12">
                        <div class="form-group">
                            ...
                            {!! Form::select('r_auth', $parserList, $servicos->r_auth, ['class' => 'form-control']) !!}
                        </div>
                    </div>
                @endif
