...
class ContasAPagarController extends Controller
{
    ...
    public function index()
    {
        ...
        $contas_a_pagar = ContasAPagar::getAll(500);
        $contas_a_pagar_count  = ContasAPagar::getAllCount(); // # -

        $controller_model  = new ContasAPagar(); // # -

        if ($user) {
            Logs::cadastrar($user->id, ($user->name . ' visualizou a lista do módulo contas_a_pagar'));
        }
        ...
        return view('contas_a_pagar.index', [
            ...
            'contas_a_pagar_count' => $contas_a_pagar_count,
    ...
    public function filter(Request $request)
    {
        try {
            ...
            $contas_a_pagar = ContasAPagar::with((new ContasAPagar())->filter_with)->select((new ContasAPagar())->getTable().'.*');
            ...
            if(isset($store['grid_fil'])){
                foreach($store['grid_fil'] as $GF_K => $GF){
                    ...
                    // ! Reforço*
                    if(count($exp_gf_k) == 2){
                        $contas_a_pagar->leftJoin($exp_gf_k[0].'_'.$exp_gf_k[1],$exp_gf_k[0].'.id',$exp_gf_k[0].'_'.$exp_gf_k[1].'.'.$exp_gf_k[0].'_id');
                        if(count($GF)){
                            foreach($GF as $GF_C => $GF_V){
                                ...
                                if(isset($store['grid_fil']['operador'][$GF_K][$GF_C]) || isset($store['grid_fil']['between'][$GF_K][$GF_C])){
                                    if($store['grid_fil']['operador'][$GF_K][$GF_C] == 'contem'){
                                        $contas_a_pagar->where($exp_gf_k[0].'_'.$exp_gf_k[1].'.'.$GF_C, "LIKE", "%" . $GF_V . "%");
                                    }elseif($store['grid_fil']['operador'][$GF_K][$GF_C] == 'entre'){
                                        $contas_a_pagar->whereBetween($exp_gf_k[0].'_'.$exp_gf_k[1].'.'.$GF_C, [$GF_V,$store['grid_fil']['between'][$GF_K][$GF_C]]);
                                    }else {
                                        $contas_a_pagar->where($exp_gf_k[0].'_'.$exp_gf_k[1].'.'.$GF_C, $store['grid_fil']['operador'][$GF_K][$GF_C], $GF_V);
                                    }
                                }else {
                                    if(is_numeric($GF_V)){
                                        $contas_a_pagar->where($exp_gf_k[0].'_'.$exp_gf_k[1].'.'.$GF_C, $GF_V);
                                    }else {
                                        $contas_a_pagar->where($exp_gf_k[0].'_'.$exp_gf_k[1].'.'.$GF_C, "LIKE", "%" . $GF_V . "%");
                                    }
                                }
                ...
                foreach ($store as $key => $value) {
                    ...
                    if (array_key_exists($key, $operador)) {
                        if ($operador[$key] == 'contem') {
                            $contas_a_pagar->where((new ContasAPagar())->getTable().'.'.$key, "LIKE", "%" . $store[$key] . "%");
                        }
                        elseif ($operador[$key] == 'entre') {
                            $contas_a_pagar->whereBetween((new ContasAPagar())->getTable().'.'.$key, [$store[$key], $between[$key]]);
                        }
                        else
                        {
                            $contas_a_pagar->where((new ContasAPagar())->getTable().'.'.$key, $operador[$key], $store[$key]);
                        }
                    }
                    else
                    {
                        if (is_numeric($store[$key])) {
                            $contas_a_pagar->where((new ContasAPagar())->getTable().'.'.$key, $store[$key]);
                        }
                        else
                        {
                            $contas_a_pagar->where((new ContasAPagar())->getTable().'.'.$key, "LIKE", "%" . $store[$key] . "%");
            ...
            {
                $contas_a_pagar = $contas_a_pagar->orderBy((new ContasAPagar())->getTable().'.'.'id', 'DESC')->groupBy((new ContasAPagar())->getTable().'.id')->limit(500)->get();
            }
            else
            {
                $contas_a_pagar = $contas_a_pagar->where(function($q) use ($r_auth) {
                    $q->where((new ContasAPagar())->getTable().'.'.'r_auth', $r_auth)
                    ->orWhere((new ContasAPagar())->getTable().'.'.'r_auth', 0);
                })->orderBy((new ContasAPagar())->getTable().'.'.'id', 'DESC')->groupBy((new ContasAPagar())->getTable().'.id')->limit(500)->get();
            }

            // # -
            $contas_a_pagar_count  = clone $contas_a_pagar;
            $contas_a_pagar_count  = $contas_a_pagar_count->count((new ContasAPagar())->getTable().'.id');
    ...
    public function create()
    {
        ...
        if ($user) {
            Logs::cadastrar($user->id, ($user->name . ' visualizou a tela de cadastro do módulo contas_a_pagar'));
        }
        ...
        return view('contas_a_pagar.add', [
            'cadastro_de_empresas_nome_fantas' => $cadastro_de_empresas_nome_fantas,
            'fornecedores_fornecedor' => $fornecedores_fornecedor,
            'formas_de_pagamentos_forma_de_pa' => $formas_de_pagamentos_forma_de_pa,
...
if ($request->hasFile("comprovante_de_pagamento")) {
    ...
    $store["comprovante_de_pagamento"] = null;
            ...
            $contas_a_pagar->r_auth = $r_auth;

            $contas_a_pagar->fill($store);

            $contas_a_pagar->save();

            if (!empty($relacionamento)) {
                $this->controllerRepository::insertRelationship($contas_a_pagar, $relacionamento, 'ContasAPagar', 'contas_a_pagar');
            }

            if ($grids) {
                $this->controllerRepository::saveGrids($contas_a_pagar, $grids, 'contas_a_pagar');
            }
...
SELECT data_do_vencimento, 'Saída', valor_pago, '0', CURRENT_DATE(), -valor_pago, ' ===> '
FROM contas_a_pagar
WHERE  id = '$contas_a_pagar->id' AND status = 1");
            ...
            if ($user) {
                Logs::cadastrar($user->id, ($user->name . ' contas_a_pagar: cadastrou ID: ' . $contas_a_pagar->id));
            }
    ...
    public function copy($id)
    {
        ...
        $contas_a_pagar = ContasAPagar::where('ID', $id)->first();

        if (env('IGNORE_COLUMNS')) {

            $hidden = env('IGNORE_COLUMNS');

            $contas_a_pagar = $contas_a_pagar->makeHidden(explode(',', $hidden));
        }

        $new = $contas_a_pagar->replicate();

        $new->push();

        Session::flash('flash_success', "Linha duplicada com sucesso!");

        return Redirect::to("/contas_a_pagar/$new->id/edit");
    }
    ...
    public function show($id)
    {
        ...
        if ($user) {
            Logs::cadastrar($user->id, ($user->name . ' visualizou o ID #' . $id . ' do módulo contas_a_pagar'));
        }
        ...
        if (isset($contas_a_pagar->Template) && $contas_a_pagar->Template->template && 1) {

            $url = $this->templateRepository::parseTemplate(['contas_a_pagar' => $contas_a_pagar]);
        ...
        {
            return view('contas_a_pagar.show', [
                'contas_a_pagar' => $contas_a_pagar,
                'cadastro_de_empresas_nome_fantas' => $cadastro_de_empresas_nome_fantas,
            'fornecedores_fornecedor' => $fornecedores_fornecedor,
            'formas_de_pagamentos_forma_de_pa' => $formas_de_pagamentos_forma_de_pa,
    ...
    public function modal($id)
    {
        ...
        return view('contas_a_pagar.modal', [
            'contas_a_pagar' => $contas_a_pagar,
            'cadastro_de_empresas_nome_fantas' => $cadastro_de_empresas_nome_fantas,
            'fornecedores_fornecedor' => $fornecedores_fornecedor,
            'formas_de_pagamentos_forma_de_pa' => $formas_de_pagamentos_forma_de_pa,
    ...
    public function ajax($value, Request $request)
    {
        ...
        if ($subForm)
        {
            if (Permissions::permissaoModerador($user))
            {
                $contas_a_pagar = ContasAPagar::where($subForm, $value);
                if(!empty($subForm2) and !empty($subForm2_value)){
                    $contas_a_pagar->where($subForm2,$subForm2_value);
                }
                if(!empty($subForm3) and !empty($subForm3_value)){
                    $contas_a_pagar->where($subForm3,$subForm3_value);
                }
                $contas_a_pagar = $contas_a_pagar->get();
            }
            else
            {
               $contas_a_pagar = ContasAPagar::where($subForm, $value)->where(function($q) use ($user) {
                    $q->whereNull('r_auth')
        ...
        else
        {
            if (Permissions::permissaoModerador($user))
            {
                $contas_a_pagar = ContasAPagar::find($value);
            }
            else
            {
               $contas_a_pagar = ContasAPagar::where(function($q) use ($user) {
                    $q->whereNull('r_auth')
                    ->orWhere('r_auth', $user->id)
                    ->orWhere('r_auth', 0);
                })->find($value);
        ...
        return Response::json($contas_a_pagar);
    }
    ...
    public function pdf($id)
    {
        ...
        return view('contas_a_pagar.pdf', [
            'contas_a_pagar' => $contas_a_pagar,
            'cadastro_de_empresas_nome_fantas' => $cadastro_de_empresas_nome_fantas,
            'fornecedores_fornecedor' => $fornecedores_fornecedor,
            'formas_de_pagamentos_forma_de_pa' => $formas_de_pagamentos_forma_de_pa,
        ...
        foreach ($collection as $key => $value) {
            ...
            try {
                ...
                if (isset($value['id']) && $value['id']) {
                    $contas_a_pagar = ContasAPagar::find($value['id']);
                }
                else
                {
                    $contas_a_pagar = new ContasAPagar();
    ...
    public function edit($id)
    {
        ...
        if ($user) {
            Logs::cadastrar($user->id, ($user->name . ' visualizou a tela de edição ID #' . $id . ' do módulo contas_a_pagar'));
        }
        ...
        return view('contas_a_pagar.edit', [
            'contas_a_pagar' => $contas_a_pagar,
            'cadastro_de_empresas_nome_fantas' => $cadastro_de_empresas_nome_fantas,
            'fornecedores_fornecedor' => $fornecedores_fornecedor,
            'formas_de_pagamentos_forma_de_pa' => $formas_de_pagamentos_forma_de_pa,
    ...
    public function update(Request $request)
    {
        try {
            ...
            $contas_a_pagar = ContasAPagar::find($store['id']);
...
WHERE  id = '$contas_a_pagar->id' AND status = 1");
            ...
            if ($user) {
                Logs::cadastrar($user->id, ($user->name . ' atualizou ID #' . $contas_a_pagar->id . ' do módulo contas_a_pagar'));
            }
...
WHERE  id = '$contas_a_pagar->id' AND status = 1");
        ...
        return $this->controllerRepository::destroy(new ContasAPagar(), $id, 'contas_a_pagar');
    }
