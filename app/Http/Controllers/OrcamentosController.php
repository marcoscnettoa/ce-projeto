...
class OrcamentosController extends Controller
{
    ...
    public function index()
    {
        ...
        $orcamentos = Orcamentos::getAll(500);
        $orcamentos_count  = Orcamentos::getAllCount(); // # -

        $controller_model  = new Orcamentos(); // # -

        if ($user) {
            Logs::cadastrar($user->id, ($user->name . ' visualizou a lista do módulo orcamentos'));
        }
    ...
    public function filter(Request $request)
    {
        try {
            ...
            $orcamentos = Orcamentos::with((new Orcamentos())->filter_with)->select((new Orcamentos())->getTable().'.*');
            ...
            if(isset($store['grid_fil'])){
                foreach($store['grid_fil'] as $GF_K => $GF){
                    ...
                    // ! Reforço*
                    if(count($exp_gf_k) == 2){
                        $orcamentos->leftJoin($exp_gf_k[0].'_'.$exp_gf_k[1],$exp_gf_k[0].'.id',$exp_gf_k[0].'_'.$exp_gf_k[1].'.'.$exp_gf_k[0].'_id');
                        if(count($GF)){
                            foreach($GF as $GF_C => $GF_V){
                                ...
                                if(isset($store['grid_fil']['operador'][$GF_K][$GF_C]) || isset($store['grid_fil']['between'][$GF_K][$GF_C])){
                                    if($store['grid_fil']['operador'][$GF_K][$GF_C] == 'contem'){
                                        $orcamentos->where($exp_gf_k[0].'_'.$exp_gf_k[1].'.'.$GF_C, "LIKE", "%" . $GF_V . "%");
                                    }elseif($store['grid_fil']['operador'][$GF_K][$GF_C] == 'entre'){
                                        $orcamentos->whereBetween($exp_gf_k[0].'_'.$exp_gf_k[1].'.'.$GF_C, [$GF_V,$store['grid_fil']['between'][$GF_K][$GF_C]]);
                                    }else {
                                        $orcamentos->where($exp_gf_k[0].'_'.$exp_gf_k[1].'.'.$GF_C, $store['grid_fil']['operador'][$GF_K][$GF_C], $GF_V);
                                    }
                                }else {
                                    if(is_numeric($GF_V)){
                                        $orcamentos->where($exp_gf_k[0].'_'.$exp_gf_k[1].'.'.$GF_C, $GF_V);
                                    }else {
                                        $orcamentos->where($exp_gf_k[0].'_'.$exp_gf_k[1].'.'.$GF_C, "LIKE", "%" . $GF_V . "%");
                                    }
                                }
                ...
                foreach ($store as $key => $value) {
                    ...
                    if (array_key_exists($key, $operador)) {
                        if ($operador[$key] == 'contem') {
                            $orcamentos->where((new Orcamentos())->getTable().'.'.$key, "LIKE", "%" . $store[$key] . "%");
                        }
                        elseif ($operador[$key] == 'entre') {
                            $orcamentos->whereBetween((new Orcamentos())->getTable().'.'.$key, [$store[$key], $between[$key]]);
                        }
                        else
                        {
                            $orcamentos->where((new Orcamentos())->getTable().'.'.$key, $operador[$key], $store[$key]);
                        }
                    }
                    else
                    {
                        if (is_numeric($store[$key])) {
                            $orcamentos->where((new Orcamentos())->getTable().'.'.$key, $store[$key]);
                        }
                        else
                        {
                            $orcamentos->where((new Orcamentos())->getTable().'.'.$key, "LIKE", "%" . $store[$key] . "%");
            ...
            {
                $orcamentos = $orcamentos->orderBy((new Orcamentos())->getTable().'.'.'id', 'DESC')->groupBy((new Orcamentos())->getTable().'.id')->limit(500)->get();
            }
            else
            {
                $orcamentos = $orcamentos->where(function($q) use ($r_auth) {
                    $q->where((new Orcamentos())->getTable().'.'.'r_auth', $r_auth)
                    ->orWhere((new Orcamentos())->getTable().'.'.'r_auth', 0);
                })->orderBy((new Orcamentos())->getTable().'.'.'id', 'DESC')->groupBy((new Orcamentos())->getTable().'.id')->limit(500)->get();
            }

            // # -
            $orcamentos_count  = clone $orcamentos;
            $orcamentos_count  = $orcamentos_count->count((new Orcamentos())->getTable().'.id');
    ...
    public function create()
    {
        ...
        if ($user) {
            Logs::cadastrar($user->id, ($user->name . ' visualizou a tela de cadastro do módulo orcamentos'));
        }

        return view('orcamentos.add', [
    ...
    public function store(Request $request)
    {
        try {
            ...
            $orcamentos->r_auth = $r_auth;

            $orcamentos->fill($store);

            $orcamentos->save();

            if (!empty($relacionamento)) {
                $this->controllerRepository::insertRelationship($orcamentos, $relacionamento, 'Orcamentos', 'orcamentos');
            }

            if ($grids) {
                $this->controllerRepository::saveGrids($orcamentos, $grids, 'orcamentos');
            }
            ...
            if ($user) {
                Logs::cadastrar($user->id, ($user->name . ' orcamentos: cadastrou ID: ' . $orcamentos->id));
            }
    ...
    public function copy($id)
    {
        ...
        $orcamentos = Orcamentos::where('ID', $id)->first();

        if (env('IGNORE_COLUMNS')) {

            $hidden = env('IGNORE_COLUMNS');

            $orcamentos = $orcamentos->makeHidden(explode(',', $hidden));
        }

        $new = $orcamentos->replicate();

        $new->push();

        Session::flash('flash_success', "Linha duplicada com sucesso!");

        return Redirect::to("/orcamentos/$new->id/edit");
    }
    ...
    public function show($id)
    {
        ...
        if ($user) {
            Logs::cadastrar($user->id, ($user->name . ' visualizou o ID #' . $id . ' do módulo orcamentos'));
        }

        if (isset($orcamentos->Template) && $orcamentos->Template->template && 1) {

            $url = $this->templateRepository::parseTemplate(['orcamentos' => $orcamentos]);
    ...
    public function ajax($value, Request $request)
    {
        ...
        if ($subForm)
        {
            if (Permissions::permissaoModerador($user))
            {
                $orcamentos = Orcamentos::where($subForm, $value);
                if(!empty($subForm2) and !empty($subForm2_value)){
                    $orcamentos->where($subForm2,$subForm2_value);
                }
                if(!empty($subForm3) and !empty($subForm3_value)){
                    $orcamentos->where($subForm3,$subForm3_value);
                }
                $orcamentos = $orcamentos->get();
            }
            else
            {
               $orcamentos = Orcamentos::where($subForm, $value)->where(function($q) use ($user) {
                    $q->whereNull('r_auth')
        ...
        else
        {
            if (Permissions::permissaoModerador($user))
            {
                $orcamentos = Orcamentos::find($value);
            }
            else
            {
               $orcamentos = Orcamentos::where(function($q) use ($user) {
                    $q->whereNull('r_auth')
                    ->orWhere('r_auth', $user->id)
                    ->orWhere('r_auth', 0);
                })->find($value);
        ...
        foreach ($collection as $key => $value) {
            ...
            try {
                ...
                if (isset($value['id']) && $value['id']) {
                    $orcamentos = Orcamentos::find($value['id']);
                }
                else
                {
                    $orcamentos = new Orcamentos();
    ...
    public function edit($id)
    {
        ...
        if ($user) {
            Logs::cadastrar($user->id, ($user->name . ' visualizou a tela de edição ID #' . $id . ' do módulo orcamentos'));
        }
    ...
    public function update(Request $request)
    {
        try {
            ...
            $orcamentos = Orcamentos::find($store['id']);
            ...
            if ($user) {
                Logs::cadastrar($user->id, ($user->name . ' atualizou ID #' . $orcamentos->id . ' do módulo orcamentos'));
            }
    ...
    public function destroy($id)
    {
        ...
        return $this->controllerRepository::destroy(new Orcamentos(), $id, 'orcamentos');
    }
