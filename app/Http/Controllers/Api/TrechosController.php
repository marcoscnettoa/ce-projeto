...
class TrechosController
{
    ...
    */
    ...
    {   
        ...
        $filter = Trechos::getAllByApi(500);
    ...
    public function filter(Request $request)
    {
        try {
                ...
                foreach ($store as $key => $value) {
                    ...
                    if (array_key_exists($key, $operador)) {
                        if ($operador[$key] == 'contem') {
                            $trechos->where($key, "LIKE", "%" . $store[$key] . "%");
                        }
                        elseif ($operador[$key] == 'entre') {
                            $trechos->whereBetween($key, [$store[$key], $between[$key]]);
                        }
                        else
                        {
                            $trechos->where($key, $operador[$key], $store[$key]);
                        }
                    }
                    else
                    {
                        if (is_numeric($store[$key])) {
                            $trechos->where($key, $store[$key]);
                        }
                        else
                        {
                            $trechos->where($key, "LIKE", "%" . $store[$key] . "%");
            ...
            {
                $trechos = $trechos->orderBy('id', 'DESC')->limit(500)->get();
            }
            else
            {
    ...
    */
    public function show($id)
    {
        try {
            ...
            if (!$trechos) {
              throw new Exception("Trechos não encontrado!", 404);
            }
            ...
            return response()->json($trechos, 200);
    ...
    */
    public function store(Request $request)
    {
        try {
            ...
            $trechos = Trechos::create($store);

            return response()->json($trechos, 201);
