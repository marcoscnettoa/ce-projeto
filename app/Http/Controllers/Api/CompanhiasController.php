...
class CompanhiasController
{
    ...
    */
    ...
    {   
        ...
        $filter = Companhias::getAllByApi(500);
    ...
    public function filter(Request $request)
    {
        try {
                ...
                foreach ($store as $key => $value) {
                    ...
                    if (array_key_exists($key, $operador)) {
                        if ($operador[$key] == 'contem') {
                            $companhias->where($key, "LIKE", "%" . $store[$key] . "%");
                        }
                        elseif ($operador[$key] == 'entre') {
                            $companhias->whereBetween($key, [$store[$key], $between[$key]]);
                        }
                        else
                        {
                            $companhias->where($key, $operador[$key], $store[$key]);
                        }
                    }
                    else
                    {
                        if (is_numeric($store[$key])) {
                            $companhias->where($key, $store[$key]);
                        }
                        else
                        {
                            $companhias->where($key, "LIKE", "%" . $store[$key] . "%");
            ...
            {
                $companhias = $companhias->orderBy('id', 'DESC')->limit(500)->get();
            }
            else
            {
    ...
    */
    public function show($id)
    {
        try {
            ...
            if (!$companhias) {
              throw new Exception("Companhias não encontrado!", 404);
            }
            ...
            return response()->json($companhias, 200);
    ...
    */
    public function store(Request $request)
    {
        try {
            ...
            $companhias = Companhias::create($store);

            return response()->json($companhias, 201);
