...
class TemplatesController
{
    ...
    */
    ...
    {   
        ...
        $filter = Templates::getAllByApi(500);
    ...
    public function filter(Request $request)
    {
        try {
                ...
                foreach ($store as $key => $value) {
                    ...
                    if (array_key_exists($key, $operador)) {
                        if ($operador[$key] == 'contem') {
                            $templates->where($key, "LIKE", "%" . $store[$key] . "%");
                        }
                        elseif ($operador[$key] == 'entre') {
                            $templates->whereBetween($key, [$store[$key], $between[$key]]);
                        }
                        else
                        {
                            $templates->where($key, $operador[$key], $store[$key]);
                        }
                    }
                    else
                    {
                        if (is_numeric($store[$key])) {
                            $templates->where($key, $store[$key]);
                        }
                        else
                        {
                            $templates->where($key, "LIKE", "%" . $store[$key] . "%");
            ...
            {
                $templates = $templates->orderBy('id', 'DESC')->limit(500)->get();
            }
            else
            {
    ...
    */
    public function show($id)
    {
        try {
            ...
            if (!$templates) {
              throw new Exception("Templates não encontrado!", 404);
            }
    ...
    */
    public function store(Request $request)
    {
        try {
            ...
            $templates = Templates::create($store);
