...
class ContasAReceber extends Model
{
    ...
    public $filter_with = ['Empresa','Cliente','Vendedor','FormaDePagamento'];
    ...
    public static function list($limit = 100, $field = false)
    {
        ...
        {
            $list = ContasAReceber::limit($limit)
                ->orderBy($field, 'ASC')
                ->pluck($field, 'contas_a_receber.id')
                ->prepend("", "");
        }
        else
        {
            $list = ContasAReceber::where(function($q) use ($user)
            {
                $user_id = 0;
                ...
                $q->where("contas_a_receber.r_auth", 0)->orWhere("contas_a_receber.r_auth", $user_id);
            })
            ->limit($limit)
            ->orderBy($field, 'ASC')
            ->pluck($field, 'contas_a_receber.id')
    ...
    public static function kanban_list($limit = 100, $field = false)
    {
        ...
        $orderBy     = 'id';
        if(Schema::hasColumn('contas_a_receber','kb_order')){
            $orderBy = 'kb_order';
        }
    ...
    public static function getAll($limit = 100)
    {
        ...
        if (Permissions::permissaoModerador($user))
        {
            $list = ContasAReceber::with(['Empresa','Cliente','Vendedor','FormaDePagamento'])->select('*')->limit($limit)
                ->orderBy('id', 'DESC')
                ->get();
        }
        else
        {
            $list = ContasAReceber::with(['Empresa','Cliente','Vendedor','FormaDePagamento'])->select('*')->where(function($q) use ($user)
            {
                $user_id = 0;
    ...
    public static function getAllCount()
    {
        ...
        if(Permissions::permissaoModerador($user))
        {
            $list_count = ContasAReceber::with([])->select('*')->count();
        }
        else
        {
            $list_count = ContasAReceber::with([])->select('*')->where(function($q) use ($user)
            {
                $user_id = 0;
                if ($user) {
                    $user_id = $user->id;
    ...
    public static function getAllByApi($limit = 100)
    {
        ...
        if (Permissions::permissaoModerador($user))
        {
            $list = ContasAReceber::select('*')->limit($limit)
                ->orderBy('id', 'DESC');
        }
        else
        {
            $list = ContasAReceber::select('*')->where(function($q) use ($user)
            {
                $user_id = 0;
    ...
    {
        return $this->belongsTo("App\Models\CadastroDeEmpresas", 'empresa');
    }
