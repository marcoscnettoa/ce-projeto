...
class MRAGIuguClientes extends Model
{
    use SoftDeletes;

    protected $table        = 'mra_g_iugu_clientes';
    ...
    public static function list($limit = 100, $field = false)
    {
        ...
        if(Permissions::permissaoModerador($user)){
            $list = MRAGIuguClientes::limit($limit)
                                        ->orderBy($field, 'DESC')
                                        ->pluck($field, 'mra_g_iugu_clientes.id')
                                        ->prepend("", "");
        }else {
            $list = MRAGIuguClientes::
                    where(function($q) use ($user){
                        $user_id = 0;
                        if ($user) {
                            $user_id = $user->id;
                        }
                        $q->where("mra_g_iugu_clientes.r_auth", 0)->orWhere("mra_g_iugu_clientes.r_auth", $user_id);
                    })
                    ->limit($limit)
                    ->orderBy($field, 'DESC')
                    ->pluck($field, 'mra_g_iugu_clientes.id')
    ...
    {
        $MRAGIuguClientes = MRAGIuguClientes::/*where('status',1)->*/orderBy('nome', 'ASC')->pluck('nome', 'mra_g_iugu_clientes.id')->prepend("---", "");
        return $MRAGIuguClientes;
    }
