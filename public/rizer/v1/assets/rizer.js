...
function setup()
{
    ...
    $('.componenteDataMesAno').datepicker({
        format: 'mm/yyyy',
        language: 'pt-BR',
        viewMode: 'months',
        minViewMode: 'months',
    ...
    $('.cep').focusout(function(){
        ...
        // # Auto Preenchimento ( ! se tem permissão para auto-preenchimento )
        if(!$(this).hasClass('auto_fill_fields')) { return; }
                        ...
                        if (form_parent.find("#" + chave).prop("type") == 'text' && key == 'id') {
                            ...
                            if (chave != input_id) {
                                if (!grid) {
                                    ...
                                    if(form_parent.find("#" + chave).hasClass('componenteData')){
                                        event_auto_fill_field_componenteData(form_parent.find("#" + chave));
                                    }
                                    if(form_parent.find("#" + chave).hasClass('componenteDataHora')){
                                        event_auto_fill_field_componenteDataHora(form_parent.find("#" + chave));
                                    }
                                }
                                else
                                {
                                    $(selfSelect).parent().parent().parent().parent().find("#" + chave).val(item);
                                    if($(selfSelect).parent().parent().parent().parent().find("#" + chave).hasClass('componenteData')){
                                        event_auto_fill_field_componenteData($(selfSelect).parent().parent().parent().parent().find("#" + chave));
                                    }
                                    if($(selfSelect).parent().parent().parent().parent().find("#" + chave).hasClass('componenteDataHora')){
                                        event_auto_fill_field_componenteDataHora($(selfSelect).parent().parent().parent().parent().find("#" + chave));
                                    }
                                }
                            }
    ...
    $('#datatable-no-buttons').on('init.dt', function () {
        if($("#datatable-no-buttons").width() > $('.dataTables_scroll').width()){ $('.table-responsive').removeClass('fixW100p'); }
    });
}

function readConditional(input)
{
    var form_parent = $(input).closest('form');
...
$(document).ready(function() {
    ...
    event_auto_fill_field_componenteData($('.componenteData'));
    event_auto_fill_field_componenteDataHora($('.componenteDataHora'));
    ...
    // # -
    $(".select_single").each(function(i,e){
        auto_fill_fields = $(e).hasClass('auto_fill_fields');
        if(auto_fill_fields){ $(e).removeClass('auto_fill_fields'); }
        $(e).trigger('change');
        if(auto_fill_fields){ $(e).addClass('auto_fill_fields'); }
    });

    $(".select_relationship").each(function(i,e){
        auto_fill_fields = $(e).hasClass('auto_fill_fields');
        if(auto_fill_fields){ $(e).removeClass('auto_fill_fields'); }
        $(e).trigger('change');
        if(auto_fill_fields){ $(e).addClass('auto_fill_fields'); }
...
function event_form_before_submit(E){
    E.find(".componenteData").each(function(e) {
        ...
        if (date.length > 0) {
            var dateSplit = date.split("/");
            var dateConvert = dateSplit[2] + '-' + dateSplit[1] + '-' + dateSplit[0];
            $(this).val(dateConvert).datepicker('update');
        }
    });
...
function event_auto_fill_field_componenteData(E){
    E.each(function(e) {
        var date = $(this).val();
        if (date.length > 0) {
            var dateSplit = date.split("-");
...
function event_auto_fill_field_componenteDataHora(E){
    E.each(function(e) {
        var date = $(this).val();
        if (date.length > 0) {
            var dataHora = date.split(" ");
...
function modal_create_event_click(E){
    E.on('click',function(){
        ...
        let listColumn      = $(this).attr('modal-list-column');
        myModalCEContent.html('');
                    ...
                    success: function(d){
                        if(d.ok && d.success.length){
                            ...
                            if(_this.attr('modal-grid')!=undefined){
                                ...
                                selectpicker_refresh_data_ajax($('select'+inputRefresh),listColumn);
                            }else {
                                console.log("entrou 2");
                                selectpicker_refresh_data_ajax(select,listColumn);
                            }
                        }
...
function selectpicker_refresh_data_ajax(ev,listColumn){
    let E          = null;
    let controller = null;
    if(typeof(ev)==='string'){
        E           = $(ev);
    ...
    if(controller != null && E != null && E[0].tagName.toLowerCase() == 'select'){
        $.get(base+'/'+controller+'/list/'+listColumn, function(data){
